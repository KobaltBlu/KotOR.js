...
export class GameEffect {
  ...
  save() {
    ...
    effectStruct
      .addField(new GFFField(GFFDataType.DWORD, 'CreatorId'))
      .setValue(typeof this.creator === 'object' ? this.creator.id : 2130706432);
    effectStruct
      .addField(new GFFField(GFFDataType.DWORD, 'SpellId'))
      .setValue(this.getSpellId() >= 0 ? this.getSpellId() : 4294967295);
    ...
    for (let i = 0; i < 6; i++) {
      ...
      objectStruct.addField(
        new GFFField(GFFDataType.DWORD, 'Value').setValue(this.getObject(i) ? this.getObject(i).id : 2130706432)
      );
