...
export class NWScriptInstance {
  ...
  init() {
    ...
    if (this.stack instanceof NWScriptStack) this.stack.dispose();
  ...
  runScript(ignoreBreakPoint: boolean = false) {
    ...
    while (this.running && !this._disposed) {
      if (this.currentInstruction) this.prevByteCode = this.currentInstruction.code;
      ...
      if (
        GameState.debugMode &&
        this.breakPoints.get(this.currentInstruction.address) &&
        !ignoreBreakPoint &&
        !stepOver
      ) {
    ...
    if (this._disposed || executionHalted) return;
  ...
  locationCompare(loc1: EngineLocation, loc2: EngineLocation) {
    return (
      ...
      loc1.position.z == loc2.position.z &&
      loc1.facing == loc2.facing
    );
  }
