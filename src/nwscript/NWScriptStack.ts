...
export class NWScriptStack {
  ...
  push(data: any, type?: any) {
    if (data instanceof NWScriptStackVariable) {
      ...
      if (type === null) console.warn('NWScriptStack', data, type);
      //throw 'Cannot push data to the stack with a type of NULL';

      if (type === undefined && type != NWScriptDataType.OBJECT) console.warn('NWScriptStack', data, type);
    ...
    for (let i = 0; i < this.stack.length; i++) {
      ...
      switch (element.type) {
        case NWScriptDataType.OBJECT:
          elementStruct
            .addField(new GFFField(GFFDataType.DWORD, 'Value'))
            .setValue(typeof element.value === 'object' ? element.value.id : 2130706432);
          break;
        ...
        if (stackElement.hasField('Value')) {
          ...
          switch (type) {
            case 4: //Object
              ...
              if (value == 0x7f000000)
                //I can confirm that this is INVALID_OBJECT_ID or OBJECT_INVALID as stated in the Bioware_Aurora_Store_Format.pdf on the old nwn.bioware.com site
                obj = undefined;
          ...
          switch (gameStruct.getType()) {
            case 0: //Effect
              stack.stack.push(
                new NWScriptStackVariable({
                  value: NWScriptStack.EffectFromStruct(gameStruct),
            ...
            case 1: //Event
              stack.stack.push(
                new NWScriptStackVariable({
                  value: NWScriptStack.EventFromStruct(gameStruct),
            ...
            case 2: //Location
              stack.stack.push(
                new NWScriptStackVariable({
                  value: NWScriptStack.LocationFromStruct(gameStruct),
            ...
            case 3: //Talent
              stack.stack.push(
                new NWScriptStackVariable({
                  value: NWScriptStack.TalentFromStruct(gameStruct),
        ...
        for (let i = 0, len = stackStructs.length; i < len; i++) {
          ...
          switch (type) {
            case NWScriptDataType.OBJECT: //Object
              ...
              if (value == 2130706432)
                //this is either OBJECT_INVALID or OBJECT_SELF
                obj = undefined;
  ...
  saveForDebugger() {
    ...
    const stringPacker = [];
    for (let i = 0; i < this.stack.length; i++) {
      if (this.stack[i].type != NWScriptDataType.STRING) {
        continue;
      }
      stringCount++;
    ...
    for (let i = 0; i < this.stack.length; i++) {
      ...
      if (element.type == NWScriptDataType.STRING) {
        dataView.setUint32(
          stackDataOffset + i * STACK_PACKET_CONSTANTS.STACK_ELEMENT_SIZE + 4,
          stringPackerIndex++,
          true
        );
        ...
        dataView.setUint32(
          stackDataOffset + i * STACK_PACKET_CONSTANTS.STACK_ELEMENT_SIZE + 4,
          typeof element.value === 'object' ? element.value.id : 2130706432,
          true
        );
  ...
  static FromDebuggerPacket(data: Uint8Array): NWScriptStack {
    ...
    for (let i = 0; i < stackLength; i++) {
      ...
      if (elementType == NWScriptDataType.STRING) {
        elementValue =
          stringDataArray[
            dataView.getUint32(stackDataOffset + i * STACK_PACKET_CONSTANTS.STACK_ELEMENT_SIZE + 4, true)
          ];
