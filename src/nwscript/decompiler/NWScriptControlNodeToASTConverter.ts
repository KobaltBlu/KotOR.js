import type {
  ...
  SwitchNode,
  SequenceNode,
} from '@/nwscript/decompiler/NWScriptControlStructureBuilder';
...
import {
  ...
  type NWScriptSwitchCaseNode,
  type NWScriptSwitchDefaultNode,
  type NWScriptExpressionStatementNode,
  type NWScriptAssignmentNode,
  type NWScriptReturnNode,
  type NWScriptBreakNode,
  type NWScriptContinueNode,
...
export class NWScriptControlNodeToASTConverter {
  ...
  private convertControlNode(
    ...
    switch (controlNode.type) {
      case 'basic_block':
        console.log(
          `[ControlNode] Processing basic_block node, block ID: ${controlNode.block.id}, instructions: ${controlNode.block.instructions.length}`
        );
        ...
        if (ifNode.condition.type === 'basic_block') {
          ...
          if (conditionInstr) {
            ...
            if (preConditionInstructions.length > 0) {
              console.log(
                `[ControlNode] Processing ${preConditionInstructions.length} pre-condition instructions in if header block ${headerBlock.id}`
              );
              ...
              for (const instr of preConditionInstructions) {
                ...
                if (instr.code === OP_RSADD) {
                  ...
                  console.log(
                    `[RSADD] Address: 0x${instr.address.toString(16).padStart(8, '0')}, SP before: ${stackPosBeforeRsadd}, Variable index: ${currentCount}`
                  );
                ...
                if (instr.code === OP_CPDOWNSP) {
                  ...
                  if (valueExpr) {
                    console.log(
                      `[CPDOWNSP] Pre-condition: Address: 0x${instr.address.toString(16).padStart(8, '0')}, SP before: ${spBefore}, Offset: ${offsetSigned}, Target pos: ${targetStackPos}`
                    );
                    console.log(
                      `[CPDOWNSP] Pre-condition: Variable positions:`,
                      Array.from(variableStackPositions.entries())
                        .map(([pos, idx]) => `pos ${pos} -> var ${idx}`)
                        .join(', ')
                    );

                    const varIndex = variableStackPositions.get(targetStackPos);
                    console.log(
                      `[CPDOWNSP] Pre-condition: Looking up variable at position ${targetStackPos}: found index ${varIndex}`
                    );
                    ...
                    for (let delta = -4; delta <= 4; delta += 4) {
                      ...
                      if (
                        nearbyVarIndex !== undefined &&
                        nearbyVarIndex >= 0 &&
                        nearbyVarIndex < this.localInits.length
                      ) {
                        const varName = `localVar_${nearbyVarIndex}`;
                        console.log(
                          `[CPDOWNSP] Pre-condition: ✓ Found nearby variable at position ${nearbyPos} (delta ${delta}): ${varName}`
                        );
                        preConditionStatements.push(NWScriptAST.createAssignment(varName, valueExpr, false));
                        continue;
                      }
                    ...
                    for (const [varPos, varIndex] of variableStackPositions.entries()) {
                      ...
                      if (distance <= 8 && varIndex >= 0 && varIndex < this.localInits.length) {
                        const varName = `localVar_${varIndex}`;
                        console.log(
                          `[CPDOWNSP] Pre-condition: ✓ Found variable by position proximity: ${varName} (distance: ${distance})`
                        );
                        preConditionStatements.push(NWScriptAST.createAssignment(varName, valueExpr, false));
                        continue;
                      }
                    ...
                    console.log(
                      `[CPDOWNSP] Pre-condition: ✗ No variable found for assignment at position ${targetStackPos}`
                    );
                  }
  ...
  private convertBasicBlock(
    ...
    console.log(
      `[Block] Processing block ${block.id} (${block.instructions.length} instructions), Function: ${functionContext?.name || 'main'}`
    );
    console.log(
      `[Block] Initial stack state - SP: ${this.stackSimulator.getStackPointer()}, Stack size: ${this.stackSimulator.getStackSize()}`
    );
    console.log(`[Block] Variable count: ${this.functionVariableCounts.get(functionContext) || 0}`);
    console.log(
      `[Block] Variable positions:`,
      Array.from(variableStackPositions.entries())
        .map(([pos, idx]) => `pos ${pos} -> var ${idx}`)
        .join(', ') || 'none'
    );
    ...
    console.log(
      `[Block] Block ${block.id} instructions:`,
      block.instructions
        .map(
          (instr) =>
            `0x${instr.address.toString(16).padStart(8, '0')} ${instr.code === OP_RSADD ? 'RSADD' : instr.code === OP_CPDOWNSP ? 'CPDOWNSP' : 'other'}`
        )
        .join(', ')
    );
    ...
    for (let i = 0; i < block.instructions.length; i++) {
      ...
      if (instruction.code === OP_RSADD) {
        ...
        console.log(
          `[RSADD] Address: 0x${instruction.address.toString(16).padStart(8, '0')}, SP before: ${stackPosBeforeRsadd}, Variable index: ${currentCount}, Function: ${functionContext?.name || 'main'}`
        );
        ...
        console.log(
          `[RSADD] Variable stack positions map:`,
          Array.from(variableStackPositions.entries())
            .map(([pos, idx]) => `pos ${pos} -> var ${idx}`)
            .join(', ')
        );
      ...
      if (instruction.code === OP_CPDOWNSP) {
        ...
        console.log(
          `[CPDOWNSP-PRE] Address: 0x${instruction.address.toString(16).padStart(8, '0')}, SP before: ${cpdownspSpBefore}, Offset: ${offsetSigned}, Target pos: ${cpdownspTargetPos}`
        );
      }
      ...
      if (instruction.code === OP_CPDOWNSP && cpdownspTargetPos !== undefined && cpdownspSpBefore !== undefined) {
        ...
        console.log(
          `[RETURN-DETECT-CHECK] Address: 0x${instruction.address.toString(16).padStart(8, '0')}, CPDOWNSP SP before: ${cpdownspSpBefore}, Target pos: ${cpdownspTargetPos}`
        );
        console.log(
          `[RETURN-DETECT-CHECK] Function: ${functionContext?.name || 'main'}, Return value offset: ${returnValueOffset}, Entry SP: ${entrySP}`
        );
        ...
        if (returnValueOffset !== undefined && entrySP !== undefined) {
          ...
          console.log(
            `[RETURN-DETECT-CHECK] Return value absolute position: ${entrySP} + ${returnValueOffset} = ${returnValueAbsolutePos}`
          );
          console.log(`[RETURN-DETECT-CHECK] CPDOWNSP absolute target position: ${cpdownspTargetPos}`);
          console.log(
            `[RETURN-DETECT-CHECK] Comparing absolute positions: ${cpdownspTargetPos} === ${returnValueAbsolutePos}? ${cpdownspTargetPos === returnValueAbsolutePos}`
          );
          console.log(
            `[RETURN-DETECT-CHECK] Current SP: ${cpdownspSpBefore}, CPDOWNSP offset: ${instruction.offset ? (instruction.offset > 0x7fffffff ? instruction.offset - 0x100000000 : instruction.offset) : 'undefined'}`
          );
          ...
          if (cpdownspTargetPos === returnValueAbsolutePos) {
            isReturnWrite = true;
            console.log(
              `[RETURN-DETECT] ✓ MATCH! Address: 0x${instruction.address.toString(16).padStart(8, '0')}, Current SP: ${cpdownspSpBefore}, Target pos: ${cpdownspTargetPos}, Entry SP: ${entrySP}, Return offset: ${returnValueOffset}, Return value absolute pos: ${returnValueAbsolutePos}`
            );

            // Get the return value expression from the stack
            returnValueExpr = this.stackSimulator.peek()?.expression;
            console.log(
              `[RETURN-DETECT] Stack top expression: ${returnValueExpr ? returnValueExpr.toNSS() : 'undefined'}`
            );
            ...
            console.log(
              `[RETURN-DETECT-CHECK] ✗ No match: CPDOWNSP absolute target ${cpdownspTargetPos} !== Return value absolute pos ${returnValueAbsolutePos} (difference: ${cpdownspTargetPos - returnValueAbsolutePos})`
            );
            console.log(
              `[RETURN-DETECT-CHECK] This suggests the return value position calculation may be incorrect, or this CPDOWNSP is writing to a different location`
            );
          }
        } else {
          console.log(
            `[RETURN-DETECT-CHECK] ✗ Missing data: returnValueOffset=${returnValueOffset}, entrySP=${entrySP}`
          );
        }
      }
      ...
      if (
        ...
        if (hasReturnSuccessor) {
          ...
          returnValueExpr = this.stackSimulator.peek()?.expression;
          console.log(
            `[RETURN-DETECT-FLOW] Address: 0x${instruction.address.toString(16).padStart(8, '0')}, target pos: ${cpdownspTargetPos} not mapped to local, block leads to return -> treating as return write`
          );
          if (returnValueExpr) {
            blockStatements.push(NWScriptAST.createReturn(returnValueExpr));
      ...
      if (instruction.code === OP_CPDOWNSP) {
        console.log(
          `[CPDOWNSP-HANDLE] Address: 0x${instruction.address.toString(16).padStart(8, '0')}, isReturnWrite: ${isReturnWrite}, cpdownspTargetPos: ${cpdownspTargetPos}`
        );
      }
      if (instruction.code === OP_CPDOWNSP && !isReturnWrite && cpdownspTargetPos !== undefined) {
        ...
        console.log(
          `[CPDOWNSP] Address: 0x${instruction.address.toString(16).padStart(8, '0')}, SP before: ${cpdownspSpBefore}, Offset: ${instruction.offset ? (instruction.offset > 0x7fffffff ? instruction.offset - 0x100000000 : instruction.offset) : 0} (0x${instruction.offset?.toString(16)}), Target pos: ${targetStackPos}`
        );
        ...
        console.log(
          `[CPDOWNSP] Expression type: ${valueExpr.type}, Value: ${JSON.stringify(valueExpr).substring(0, 100)}`
        );
        console.log(
          `[CPDOWNSP] Variable stack positions map:`,
          Array.from(variableStackPositions.entries())
            .map(([pos, idx]) => `pos ${pos} -> var ${idx}`)
            .join(', ')
        );
      ...
      if (expr) {
        ...
        if (expr.type === 'function_call') {
          ...
          for (let j = i + 1; j < lookAheadLimit; j++) {
            ...
            if (
              futureInstr.code === OP_EQUAL ||
              futureInstr.code === OP_NEQUAL ||
              futureInstr.code === OP_GT ||
              futureInstr.code === OP_GEQ ||
              futureInstr.code === OP_LT ||
              futureInstr.code === OP_LEQ ||
              futureInstr.code === OP_ADD ||
              futureInstr.code === OP_SUB ||
              futureInstr.code === OP_MUL ||
              futureInstr.code === OP_DIV ||
              futureInstr.code === OP_LOGANDII ||
              futureInstr.code === OP_LOGORII ||
  ...
  private convertIfNode(node: IfNode, functionContext: NWScriptFunction | null): NWScriptIfNode {
    ...
    if (node.body.type === 'basic_block') {
      ...
      if (hasLogAndII && node.condition.type === 'basic_block') {
        console.log(
          `[convertIfNode] Attempting cross-block AND condition extraction from block ${node.condition.block.id} through block ${bodyBlock.id}`
        );
        ...
        if (combinedCondition) {
          ...
          if (successorsArray.length > 0) {
            ...
            console.log(
              `[convertIfNode] Body block ${bodyBlock.id} has successor block ${nextBlock.id}, checking for nested structure...`
            );
            ...
            if (nextBlock.conditionInstruction) {
              console.log(
                `[convertIfNode] Successor block ${nextBlock.id} has condition instruction, likely a nested if structure`
              );
    ...
    if (thenBody.statements.length === 0 && node.body.type === 'basic_block') {
      ...
      if (hasLogAndII) {
        console.log(
          `[convertIfNode] WARNING: Body block ${bodyBlock.id} contains LOGANDII and generated empty body. The actual body should be the nested structure starting from block ${Array.from(bodyBlock.successors)[0]?.id || 'unknown'}`
        );
      }
    }
    ...
    if (conditionNode.type === 'basic_block') {
      ...
      if (block.conditionInstruction) {
        ...
        if (stackTop) {
          ...
          console.log(
            `[extractConditionFromBlock] Stack size: ${this.stackSimulator.getStackSize()}, SP: ${this.stackSimulator.getStackPointer()}`
          );

          // Check if this is just a variable (which might be wrong)
          if (stackTop.expression.type === 'variable') {
            console.log(
              `[extractConditionFromBlock] WARNING: Condition is just a variable ${stackTop.expression.variableName}, might be incorrect`
            );
            console.log(
              `[extractConditionFromBlock] Block ${block.id} instructions:`,
              block.instructions
                .map((instr: NWScriptInstruction) => `${instr.address.toString(16).padStart(8, '0')} ${instr.codeName}`)
                .join(', ')
            );
            const blockRange = block.getAddressRange();
            console.log(
              `[extractConditionFromBlock] Block ${block.id} startAddress: ${blockRange.start.toString(16)}, endAddress: ${blockRange.end.toString(16)}`
            );
            console.log(
              `[extractConditionFromBlock] Condition instruction at: ${conditionInstr.address.toString(16).padStart(8, '0')} ${conditionInstr.codeName}`
            );
            ...
            if (conditionIndex > 0) {
              ...
              console.log(
                `[extractConditionFromBlock] Instructions before condition:`,
                instructionsBeforeCondition
                  .map(
                    (instr: NWScriptInstruction) => `${instr.address.toString(16).padStart(8, '0')} ${instr.codeName}`
                  )
                  .join(', ')
              );
              ...
              if (equalIndex >= 0 && cptopspIndex >= 0 && cptopspIndex > equalIndex) {
                console.log(
                  `[extractConditionFromBlock] Found EQUAL at index ${equalIndex} followed by CPTOPSP at index ${cptopspIndex} - this is a short-circuit pattern`
                );
                console.log(
                  `[extractConditionFromBlock] Need to reconstruct condition from EQUAL result, not CPTOPSP result`
                );
                ...
                if (
                  ...
                  console.log(
                    `[extractConditionFromBlock] Body block ${bodyBlock.id} startAddress: ${bodyRange.start.toString(16)}, endAddress: ${bodyRange.end.toString(16)}`
                  );
                  console.log(
                    `[extractConditionFromBlock] Body block ${bodyBlock.id} instructions:`,
                    bodyBlock.instructions
                      .map(
                        (instr: NWScriptInstruction) =>
                  ...
                  let hasLogAndII = bodyBlock.instructions.some(
                    (instr: NWScriptInstruction) => instr.code === OP_LOGANDII
                  );
                  console.log(
                    `[extractConditionFromBlock] Body block ${bodyBlock.id} has LOGANDII in instruction list: ${hasLogAndII}`
                  );
                  ...
                  if (bodyRange.end >= logAndIIAddr && bodyRange.start <= logAndIIAddr) {
                    console.log(
                      `[extractConditionFromBlock] LOGANDII at ${logAndIIAddr.toString(16)} is within body block ${bodyBlock.id} address range`
                    );
                    ...
                    if (!logAndIIInstr) {
                      console.log(
                        `[extractConditionFromBlock] WARNING: LOGANDII at ${logAndIIAddr.toString(16)} is in address range but not in instruction list!`
                      );
                      ...
                      if (logAndII) {
                        console.log(
                          `[extractConditionFromBlock] Found LOGANDII instruction in CFG at ${logAndIIAddr.toString(16)}`
                        );
                  ...
                  if (hasLogAndII) {
                    ...
                    if (combinedCondition) {
                      console.log(
                        `[extractConditionFromBlock] Successfully extracted cross-block AND condition: ${combinedCondition.toNSS()}`
                      );
                  ...
                  console.log(
                    `[extractConditionFromBlock] Attempting to reconstruct condition from EQUAL and body block...`
                  );
                  ...
                  if (equalInstrIndex >= 0) {
                    ...
                    const varStackPositions =
                      this.functionVariableStackPositions.get(functionContext) || new Map<number, number>();
                    ...
                    console.log(
                      `[extractConditionFromBlock] Starting condition reconstruction with varStackPositions:`,
                      Array.from(varStackPositions.entries())
                        .map(([pos, idx]) => `pos ${pos} -> var ${idx}`)
                        .join(', ')
                    );
                    for (let i = 0; i <= equalInstrIndex; i++) {
                      ...
                      if (instr.code === OP_CPTOPSP || instr.code === OP_CONST || instr.code === OP_EQUAL) {
                        ...
                        console.log(
                          `[extractConditionFromBlock] Before ${instr.codeName} at ${instr.address.toString(16).padStart(8, '0')}: expression stack size=${exprStackBefore}`
                        );
                      ...
                      if (instr.code === OP_CPTOPSP) {
                        ...
                        console.log(
                          `[extractConditionFromBlock] CPTOPSP at ${instr.address.toString(16).padStart(8, '0')}: SP=${spBefore}, offset=${offsetSigned}, sourcePos=${sourceStackPos}`
                        );
                        ...
                        if (varIdx !== undefined) {
                          ...
                          console.log(
                            `[extractConditionFromBlock] CPTOPSP: No variable found at position ${sourceStackPos}`
                          );
                        }
                      }
                      ...
                      if (
                        ...
                        if (instr.code === OP_CPTOPSP || instr.code === OP_CONST || instr.code === OP_EQUAL) {
                          ...
                          console.log(
                            `[extractConditionFromBlock] After ${instr.codeName} at ${instr.address.toString(16).padStart(8, '0')}: expression stack size=${exprStackAfter}`
                          );
                          if (exprStackAfter > 0) {
                            const topExpr = tempExprBuilder.peek();
                            if (topExpr) {
                          ...
                          if (instr.code === OP_EQUAL && exprStackAfter > 0) {
                            ...
                            if (topExpr && topExpr.type === 'comparison' && topExpr.left && topExpr.right) {
                              console.log(
                                `[extractConditionFromBlock]   EQUAL left: ${topExpr.left.toNSS()}, right: ${topExpr.right.toNSS()}`
                              );
                            }
                          }
                        }
                    ...
                    if (firstCondition) {
                      console.log(
                        `[extractConditionFromBlock] Reconstructed first condition from EQUAL: ${firstCondition.toNSS()}`
                      );
                      ...
                      console.log(
                        `[extractConditionFromBlock] Processing body block instructions for second condition...`
                      );
                      for (const instr of bodyBlock.instructions) {
                        ...
                        if (instr.code === OP_CPTOPSP || instr.code === OP_GT) {
                          ...
                          console.log(
                            `[extractConditionFromBlock] Body: Before ${instr.codeName} at ${instr.address.toString(16).padStart(8, '0')}: expression stack size=${exprStackBefore}`
                          );
                          if (exprStackBefore > 0) {
                        ...
                        if (instr.code === OP_CPTOPSP) {
                          ...
                          const sourceStackPos = spBefore + offsetSigned;
                          console.log(
                            `[extractConditionFromBlock] Body: CPTOPSP at ${instr.address.toString(16).padStart(8, '0')}: SP=${spBefore}, offset=${offsetSigned}, sourcePos=${sourceStackPos}`
                          );
                          const varIdx = varStackPositions.get(sourceStackPos);
                          if (varIdx !== undefined) {
                            ...
                            console.log(
                              `[extractConditionFromBlock] Body: CPTOPSP: No variable found at position ${sourceStackPos}`
                            );
                          }
                        }
                        ...
                        if (instr.code === OP_GT) {
                          ...
                          console.log(
                            `[extractConditionFromBlock] Body: After GT at ${instr.address.toString(16).padStart(8, '0')}: expression stack size=${exprStackAfter}`
                          );
                          if (exprStackAfter > 0) {
                            const topExpr = bodyExprBuilder.peek();
                            if (topExpr) {
                              ...
                              if (topExpr.type === 'comparison' && topExpr.left && topExpr.right) {
                                console.log(
                                  `[extractConditionFromBlock] Body:   GT left: ${topExpr.left.toNSS()}, right: ${topExpr.right.toNSS()}`
                                );
                              }
                            }
                          }
                          ...
                          if (instr.code === OP_CPTOPSP) {
                            const exprStackAfter = bodyExprBuilder.getStackSize();
                            console.log(
                              `[extractConditionFromBlock] Body: After CPTOPSP at ${instr.address.toString(16).padStart(8, '0')}: expression stack size=${exprStackAfter}`
                            );
                            if (exprStackAfter > 0) {
                              const topExpr = bodyExprBuilder.peek();
                              if (topExpr) {
                      ...
                      if (secondCondition) {
                        console.log(
                          `[extractConditionFromBlock] Extracted second condition from body block: ${secondCondition.toNSS()}`
                        );
  ...
  private extractCrossBlockANDCondition(
    ...
    console.log(
      `[extractCrossBlockANDCondition] Starting extraction from block ${firstBlock.id} to block ${secondBlock.id}`
    );
    ...
    if (logAndIIIndex >= 0) {
      ...
      console.log(
        `[extractCrossBlockANDCondition] Found LOGANDII in block ${secondBlock.id} at index ${logAndIIIndex}`
      );
    } else {
      // Check predecessors of second block
      console.log(
        `[extractCrossBlockANDCondition] LOGANDII not in block ${secondBlock.id}, checking predecessors:`,
        Array.from(secondBlock.predecessors)
          .map((b: NWScriptBasicBlock) => b.id)
          .join(', ')
      );
      for (const predecessor of secondBlock.predecessors) {
        ...
        if (idx >= 0) {
          ...
          logAndIIIndex = idx;
          console.log(
            `[extractCrossBlockANDCondition] Found LOGANDII in predecessor block ${predecessor.id} at index ${idx}`
          );
          break;
        }
      }
  ...
  private setupFunctionContext(func: NWScriptFunction): void {
    ...
    if (!this.functionReturnValueOffsets.has(func)) {
      ...
      console.log(
        `[setupFunctionContext] Function ${func.name}: Calculated return value absolute position = ${returnValuePos}`
      );
      ...
      if (returnValuePos !== undefined && entrySP !== undefined) {
        ...
        console.log(
          `[setupFunctionContext] ✓ Function ${func.name}: Entry SP = ${entrySP}, Return value absolute pos = ${returnValuePos}, Return value offset = ${returnValueOffset}`
        );
      } else {
        console.log(
          `[setupFunctionContext] ✗ Function ${func.name}: Missing entrySP (${entrySP}) or returnValuePos (${returnValuePos})`
        );
      }
    } else {
      ...
      console.log(
        `[setupFunctionContext] Function ${func.name}: Using cached return value offset = ${existingOffset}, entry SP = ${existingEntrySP}`
      );
    }
  }
  ...
  private calculateFunctionEntryStackPointer(func: NWScriptFunction): number | undefined {
    ...
    if (func.jsrInstruction) {
      ...
      console.log(
        `[calculateFunctionEntryStackPointer] Function ${func.name}: JSR at ${jsrInstr.address.toString(16)}, Entry SP (at JSR) = ${entrySP}`
      );
      return entrySP;
    } else {
      ...
      if (jsrInstr) {
        ...
        const entrySP = tempSim.getStackPointer();
        console.log(
          `[calculateFunctionEntryStackPointer] Function ${func.name}: Found JSR at ${jsrInstr.address.toString(16)} calling entry ${funcEntryAddress.toString(16)}, Entry SP (at JSR) = ${entrySP}`
        );
        return entrySP;
      } else {
        ...
        const entrySP = tempSim.getStackPointer();
        console.log(
          `[calculateFunctionEntryStackPointer] Function ${func.name}: Entry address = ${funcEntryAddress.toString(16)}, Entry SP = ${entrySP}`
        );
        return entrySP;
      }
    }
  ...
  private calculateReturnValuePosition(func: NWScriptFunction): number | undefined {
    if (!func.jsrInstruction) {
      ...
      if (func.isMain && func.entryBlock) {
        ...
        if (rsaddInstr) {
          ...
          console.log(
            `[calculateReturnValuePosition] Main function: SP before RSADD at ${rsaddInstr.address.toString(16)} = ${spBeforeRsadd}`
          );
          ...
          console.log(
            `[calculateReturnValuePosition] Main function: SP at RSADD = ${spAtRsadd}, return value position = ${returnValuePos}`
          );
    ...
    if (!rsaddInstr) {
      console.log(
        `[calculateReturnValuePosition] Function ${func.name}: No RSADD found before JSR at ${jsrInstr.address.toString(16)}`
      );
      return undefined;
    }
    ...
    const returnValuePos = spAtRsadd;
    console.log(
      `[calculateReturnValuePosition] Function ${func.name}: SP at RSADD (before RSADD executes) = ${spAtRsadd}`
    );
    // Process RSADD to see what SP becomes after
    tempSim.processInstruction(rsaddInstr);
    const spAfterRsadd = tempSim.getStackPointer();
    ...
    console.log(
      `[calculateReturnValuePosition] Function ${func.name}: Found RSADD at ${rsaddInstr.address.toString(16)} before JSR at ${jsrInstr.address.toString(16)}, return value position = ${returnValuePos}`
    );
    return returnValuePos;
  }
