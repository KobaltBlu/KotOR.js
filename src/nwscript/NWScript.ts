...
export class NWScript {
  ...
  constructor(dataOrFile?: string | Uint8Array) {
    this.actionsMap = GameState.GameKey == GameEngineType.TSL ? NWScriptDefK2.Actions : NWScriptDefK1.Actions;
  ...
  verifyNCS(reader: BinaryReader) {
    reader.seek(0);
    if (this.isVerified || reader.readChars(8) == 'NCS V1.0') return (this.isVerified = true);
  ...
  toAssembly(): string {
    ...
    const sortedInstructions = Array.from(this.instructions.values()).sort((a, b) => a.address - b.address);
