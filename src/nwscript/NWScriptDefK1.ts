  ...
  Actions: any = {
    ...
    255:{
      ...
      action: function(args: any, _instr: any, action: any){
        ...
        if((args[1]) instanceof ModuleObject){
          if(args[0] != ''){
            GameState.InGameDialog.StartConversation(args[0], this.caller, args[1]);
            return 1;
          }else if(this.caller._conversation){
            GameState.InGameDialog.StartConversation(this.caller._conversation, this.caller, args[1]);
            (args[1])._conversation = '';
            return 1;
          }else if(this.caller.conversation){
            GameState.InGameDialog.StartConversation(this.caller.conversation, this.caller, args[1]);
            return 1;
          }else if(this.listenPatternSpeaker.conversation){
            GameState.InGameDialog.StartConversation(this.listenPatternSpeaker.conversation, this.caller, this.listenPatternSpeaker);
            return 1;
          }else{
    ...
    461:{
      ...
      args: ["int"],
      action: function(args: any, _instr: any, action: any){
        GameState.InGameDialog.SetPlaceableCamera(args[0]);
      }
    },
    462:{
    ...
    719:{
      ...
      action: function(args: any, _instr: any, action: any){
        //console.log('SetGlobalFadeIn', GameState.FadeOverlay.holdForScript);
        setTimeout( () => {
          GameState.FadeOverlay.holdForScript = false;
          //console.log('SetGlobalFadeIn', GameState.FadeOverlay.holdForScript);
          GameState.FadeOverlay.FadeIn(args[1], args[2], args[3], args[4]);
        }, args[0] * 1000);
    ...
    720:{
      ...
      action: function(args: any, _instr: any, action: any){
        setTimeout( () => {
          GameState.FadeOverlay.FadeOut(args[1], args[2], args[3], args[4]);
        }, args[0] * 1000);
      }
    },
    721:{
