...
export class NWScriptCompiler {
  ...
  triggerEventListener(type: string) {
    ...
    if (evList) {
      for (let i = 0; i < evList.length; i++) {
        if (typeof evList[i] == 'function') evList[i]();
      }
      return true;
    }
    return false;
  ...
  compilePass(funcMain: CompilerFunctionNode, returnInt: boolean = false): ByteArray[] {
    ...
    buffers.push(this.writeJSR(this.getInstructionLength(OP_JSR) + this.getInstructionLength(OP_RETN)));
    buffers.push(this.writeRETN());
  ...
  compileStatement(statement: CompilerStatementNode | any) {
    if (statement) {
      switch (statement.type) {
        case 'literal':
          return this.compileLiteral(statement);
        case 'array_literal':
          return this.compileArrayLiteral(statement);
        case 'variable':
          return this.compileVariable(statement);
        case 'variableList':
          return this.compileVariableList(statement);
        case 'variable_reference':
          return this.compileVariable(statement);
        case 'argument':
          return this.compileArgument(statement);
        case 'struct':
          return this.compileStruct(statement);
        case 'property':
          return this.compileStructProperty(statement);
        case 'compare':
          return this.compileCompare(statement);
        case 'function':
          return this.compileFunction(statement);
        case 'function_call':
          return this.compileFunctionCall(statement);
        case 'block':
          return this.compileAnonymousBlock(statement);
        case 'return':
          return this.compileReturn(statement);
        case 'if':
          return this.compileIf(statement);
        case 'switch':
          return this.compileSwitch(statement);
        case 'do':
          return this.compileDoWhileLoop(statement);
        case 'while':
          return this.compileWhileLoop(statement);
        case 'for':
          return this.compileForLoop(statement);
        case 'incor':
          return this.compileINCOR(statement);
        case 'xor':
          return this.compileEXCOR(statement);
        case 'booland':
          return this.compileBOOLAND(statement);
        case 'shift':
          return this.compileShift(statement);
        case 'comp':
          return this.compileComp(statement);
        case 'add':
          return this.compileAdd(statement);
        case 'sub':
          return this.compileSub(statement);
        case 'mul':
          return this.compileMul(statement);
        case 'div':
          return this.compileDiv(statement);
        case 'neg':
          return this.compileNEG(statement);
        case 'not':
          return this.compileNOT(statement);
        case 'inc':
          return this.compileINC(statement);
        case 'dec':
          return this.compileDEC(statement);
        case 'continue':
          return this.compileContinue(statement);
        case 'break':
          return this.compileBreak(statement);
        case 'assign':
          return this.compileAssign(statement);
        default:
          console.error('unhandled statement', statement.type);
          console.log(statement);
      }
    }
  ...
  getDataType(value?: any): any {
    if (typeof value == 'object') {
      ...
      if (value.type == 'variable') {
        return value.datatype || value?.variable_reference?.datatype || value?.variable_reference?.datatype;
      }
      if (value.type == 'variableList') {
        ...
        if (statement.declare === true) {
          ...
          if (statement.struct_reference && statement.variable_reference) {
            if (statement.value) {
              ...
              const propertyStackPointer =
                statement.struct_reference.stackPointer + statement.variable_reference.offsetPointer;
              if (statement.struct_reference.is_global) {
                buffers.push(
                  this.writeCPDOWNBP(
                    propertyStackPointer - this.basePointer,
                    this.getDataTypeStackLength(statement.datatype)
                  )
                );
              } else {
                buffers.push(
                  this.writeCPDOWNSP(
                    propertyStackPointer - this.stackPointer,
                    this.getDataTypeStackLength(statement.datatype)
                  )
                );
              }
              ...
              if (statement.struct_reference.is_global) {
                buffers.push(
                  this.writeCPTOPBP(
                    statement.struct_reference.stackPointer - this.basePointer,
                ...
                buffers.push(
                  this.writeCPTOPSP(
                    statement.struct_reference.stackPointer - this.stackPointer,
                    statement.struct_reference.struct_reference.structDataLength
                  )
                );
              }
        ...
        if (statement.declare === true) {
          ...
          if (statement.value) {
            ...
            if (statement.variable_reference.is_global) {
              ...
              const arg_stack_pointer =
                this.stackPointer -
                this.scope.block.preStatementsStackPointer +
                statement.variable_reference.stackPointer;
            ...
            if (statement.is_global) {
              ...
              if (varRef.is_engine_constant) {
                ...
                const arg_stack_pointer =
                  this.stackPointer - this.scope.block.preStatementsStackPointer + varRef.stackPointer;
    ...
    if (statement && statement.type == 'assign') {
      ...
      if (left.type == 'property') {
        ...
        if (isGlobal) {
          buffers.push(
            this.writeCPDOWNBP(
              varRef.stackPointer - this.basePointer + propRef.offsetPointer,
              this.getDataTypeStackLength(statement.datatype)
            )
          );
        } else {
          buffers.push(
            this.writeCPDOWNSP(
              varRef.stackPointer - this.stackPointer + propRef.offsetPointer,
              this.getDataTypeStackLength(statement.datatype)
            )
          );
        }
        ...
        if (varRef.is_global) {
          buffers.push(
            this.writeCPDOWNBP(varRef.stackPointer - this.basePointer, this.getDataTypeStackLength(statement.datatype))
          );
        } else {
          buffers.push(
            this.writeCPDOWNSP(varRef.stackPointer - this.stackPointer, this.getDataTypeStackLength(statement.datatype))
          );
        }
      }
  ...
  compileFunction(block: CompilerFunctionNode): Uint8Array {
    ...
    if (block.arguments.length) {
      for (let i = 0; i < block.arguments.length; i++) {
        const arg: any = block.arguments[i];
        arg.index = i;
    ...
    for (let i = 0; i < block.statements.length; i++) {
      ...
      if (stmt && typeof stmt === 'object') {
        (stmt as any).statement_context = 'statement';
      }
    ...
    if (statement && statement.type == 'function_call') {
      ...
      for (let i = 0; i < __arguments.length; i++) {
        ...
        if (arg_ref.datatype.value == 'action') {
          ...
          buffers.push(
            this.writeJMP(
              this.getInstructionLength(OP_JMP) + this.getStatementLength(arg) + this.getInstructionLength(OP_RETN)
            )
          );
  ...
  compileAdd(statement: any) {
    ...
    if (statement && statement.type == 'add') {
      ...
      if (
        this.getDataType(statement.left).unary == NWCompileDataTypes.I &&
        this.getDataType(statement.right).unary == NWCompileDataTypes.I
      ) {
        buffers.push(this.writeADD(NWCompileDataTypes.II));
      } else if (
        this.getDataType(statement.left).unary == NWCompileDataTypes.I &&
        this.getDataType(statement.right).unary == NWCompileDataTypes.F
      ) {
        buffers.push(this.writeADD(NWCompileDataTypes.IF));
      } else if (
        this.getDataType(statement.left).unary == NWCompileDataTypes.F &&
        this.getDataType(statement.right).unary == NWCompileDataTypes.I
      ) {
        ...
        this.getDataType(statement.left).unary == NWCompileDataTypes.S &&
        this.getDataType(statement.right).unary == NWCompileDataTypes.S
      ) {
        buffers.push(this.writeADD(NWCompileDataTypes.SS));
      } else if (
        this.getDataType(statement.left).unary == NWCompileDataTypes.V &&
        this.getDataType(statement.right).unary == NWCompileDataTypes.V
      ) {
    ...
    if (statement && statement.type == 'compare') {
      ...
      if (statement.left?.type === 'variable_reference' && leftBuf.length === 0) {
        ...
        const dtSize = this.getDataTypeStackLength(
          statement.left.datatype || varRef?.datatype || this.getDataType(statement.left)
        );
        if (varRef && typeof varRef.stackPointer === 'number') {
          if (varRef.is_global) {
            buffers.push(this.writeCPTOPBP(varRef.stackPointer - this.basePointer, dtSize));
      ...
      if (statement.operator.value == '==') {
        if (lUnary == NWCompileDataTypes.I && rUnary == NWCompileDataTypes.I) {
          ...
          if (
            engine_type_left !== undefined &&
            engine_type_right !== undefined &&
            engine_type_left >= 0x10 &&
            engine_type_left <= 0x1f &&
            engine_type_right >= 0x10 &&
            engine_type_right <= 0x1f
          ) {
        ...
        if (lUnary == NWCompileDataTypes.I && rUnary == NWCompileDataTypes.I) {
          ...
          console.error(
            'Unsupported: LOGANDII datatypes',
            this.getDataType(statement.left),
            this.getDataType(statement.right)
          );
        }
      } else if (statement.operator.value == '||') {
        if (lUnary == NWCompileDataTypes.I && rUnary == NWCompileDataTypes.I) {
  ...
  compileIf(statement: any) {
    ...
    if (statement && statement.type == 'if') {
      ...
      const ifelses: any[] = ([] as any[])
        .concat([statement], statement.elseIfs || [], statement.else || [])
        .filter(Boolean);
  ...
  compileDoWhileLoop(statement: any) {
    ...
    if (statement && statement.type == 'do') {
      ...
      const conds = Array.isArray(statement.condition)
        ? statement.condition
        : statement.condition
          ? [statement.condition]
          : [];
      ...
      buffers.push(this.writeJZ(this.getInstructionLength(OP_JZ) + this.getInstructionLength(OP_JMP)));

      //JMP back to the beginning of the DO-While statement
      buffers.push(this.writeJMP(-(this.scope.bytes_written - statement.block_start)));
  ...
  compileWhileLoop(statement: any) {
    ...
    if (statement && statement.type == 'while') {
      ...
      //If the condition is false Jump out of the loop
      buffers.push(this.writeJZ(statement.block_end ? statement.block_end - this.scope.bytes_written : 0x7fffffff));
  ...
  compileForLoop(statement: any) {
    ...
    if (statement && statement.type == 'for') {
      ...
      if (statement.initializer && typeof statement.initializer === 'object') {
        (statement.initializer as any).statement_context = 'statement';
      }
      ...
      buffers.push(
        this.writeJMP(statement.condition_start ? -(this.scope.bytes_written - statement.condition_start) : 0x7fffffff)
      );
  ...
  compileContinue(statement: CompilerContinueNode) {
    ...
    if (active_loop) {
      ...
      buffers.push(
        this.writeJMP(
          active_loop.statement.continue_start
            ? -(this.scope.bytes_written - active_loop.statement.continue_start)
            : 0x7fffffff
        )
      );
    } else {
    ...
    if (statement && statement.type == 'inc') {
      ...
      if (varRef.is_global) {
        if (!statement.postfix) {
          buffers.push(this.writeINCIBP(varRef.stackPointer - this.stackPointer));
        }
        buffers.push(
          this.writeCPTOPBP(varRef.stackPointer - this.basePointer, this.getDataTypeStackLength(varRef.datatype))
        );
        if (statement.postfix) {
          buffers.push(this.writeINCIBP(varRef.stackPointer - this.basePointer));
        }
      } else {
        if (!statement.postfix) {
          buffers.push(this.writeINCISP(varRef.stackPointer - this.stackPointer));
        }
        buffers.push(
          this.writeCPTOPSP(varRef.stackPointer - this.stackPointer, this.getDataTypeStackLength(varRef.datatype))
        );
        if (statement.postfix) {
          buffers.push(this.writeINCISP(varRef.stackPointer - this.stackPointer));
        }
  ...
  compileDEC(statement: any): Uint8Array {
    ...
    if (statement && statement.type == 'dec') {
      ...
      if (varRef.is_global) {
        if (!statement.postfix) {
          buffers.push(this.writeDECIBP(varRef.stackPointer - this.basePointer));
        }
        buffers.push(
          this.writeCPTOPBP(varRef.stackPointer - this.basePointer, this.getDataTypeStackLength(varRef.datatype))
        );
        ...
        if (!statement.postfix) {
          buffers.push(this.writeDECISP(varRef.stackPointer - this.stackPointer));
        }
        buffers.push(
          this.writeCPTOPSP(varRef.stackPointer - this.stackPointer, this.getDataTypeStackLength(varRef.datatype))
        );
  ...
  writeADD(type = 0x20) {
    ...
    if (type == NWCompileDataTypes.II) {
      this.stackPointer += 4;
      this.stackPointer -= 4 * 2;
    }
    if (type == NWCompileDataTypes.IF) {
      this.stackPointer += 4;
      this.stackPointer -= 4 * 2;
    }
    if (type == NWCompileDataTypes.FI) {
      this.stackPointer += 4;
      this.stackPointer -= 4 * 2;
    }
    if (type == NWCompileDataTypes.FF) {
      this.stackPointer += 4;
      this.stackPointer -= 4 * 2;
    }
    if (type == NWCompileDataTypes.SS) {
      this.stackPointer += 4;
      this.stackPointer -= 4 * 2;
    }
    if (type == NWCompileDataTypes.VV) {
      this.stackPointer += 12;
      this.stackPointer -= 12 * 2;
    }
  ...
  writeMUL(type = 0x20) {
    ...
    if (type == NWCompileDataTypes.VF) {
      this.stackPointer += 12;
      this.stackPointer -= 12 + 4;
    }
    if (type == NWCompileDataTypes.FV) {
      this.stackPointer += 12;
      this.stackPointer -= 4 + 12;
    }
...
class NWScriptScope {
  ...
  getTopContinueableNestedState() {
    return this.nested_states
      .slice(0)
      .reverse()
      .find((s) => {
        return s.statement.type == 'for' || s.statement.type == 'while' || s.statement.type == 'do';
      });
  }
