...
export class NWScriptParser {
  ...
  postProcessFunctionDefinition(statement: FunctionNode | EngineAction): FunctionNode | EngineAction {
  ...
  initializeNWScript() {
    ...
    const hp = new NWScriptASTBuilder(this.nwscript_source, {
  ...
  parseAST(script: string): ProgramNode | undefined {
    try {
      const hp = new NWScriptASTBuilder(script, {
        engineTypes: this.engine_types.map((t) => ({ name: t.name, unary: t.datatype.unary })),
      });
      return hp.parseAST();
    } catch (e: unknown) {
      console.log(e);
      const err = e as { type?: string; statement?: unknown; offender?: unknown };
      this.errors.push({
        type: (err?.type === 'compile' || err?.type === 'parse' ? err.type : 'parse') as 'compile' | 'parse',
  ...
  parseScript(script?: string) {
    ...
    try {
      ...
      const err = e as { statement?: unknown; offender?: unknown };
      this.errors.push({
        type: 'parse',
        message: e instanceof Error ? e.message : 'Parse error',
        statement: err?.statement,
  ...
  getVariableByName(
    name: any = ''
  ): SemanticVariableNode | SemanticStructPropertyNode | SemanticStructNode | SemanticArgumentNode | undefined {
  ...
  isNameInUse(name = '') {
    ...
    return (
      isEngineActionName ||
      isScriptFunctionNameInUse ||
      isScriptStructNameInUse ||
      (isScriptVariableNameInUse && !isScriptArgumentNameInUse)
    );
  }
  ...
  parseProgramNode(program: ProgramNode): SemanticProgramNode {
    ...
    const main = statements.find(
      (s: any) => s.type == 'function' && s.name == 'main' && s.returntype && this.isDataType(s.returntype, 'void')
    );

    //detect int startingConditional()
    const startingConditional = program.statements.find(
      (s: any) =>
        s.type == 'function' && s.name == 'StartingConditional' && s.returntype && this.isDataType(s.returntype, 'int')
    );

    //detect the global function headers
    const global_functions_headers = statements.filter(
      (s: any) => s.type == 'function' && s.header_only && s.name != 'main' && s.name != 'StartingConditional'
    ) as FunctionNode[];

    //detect the global functions with header and body
    const global_functions = statements.filter(
      (s: any) => s.type == 'function' && !s.header_only && s.name != 'main' && s.name != 'StartingConditional'
    ) as FunctionNode[];
    ...
    if (main && startingConditional) {
      this.throwError(
        'You cannot have both `void main()` and `int StartingConditional()` declared in the same script',
        program,
        program
      );
    } else if (!main && !startingConditional) {
      this.throwError(
        'You cannot compile a script without either a void main() or int StartingConditional() declared in the script',
        program,
        program
      );
    }
    ...
    this.program.statements = statements
      .filter((s: any) => s.type != 'function')
      .map((s) => this.parseASTStatement(s) as SemanticStatementNode);
  ...
  parseFunctionCallNode(statement: FunctionCallNode): SemanticFunctionCallNode {
    ...
    if (engineAction) {
      ...
      semanticNode.function_reference = engineAction as EngineActionRef;
    } else {
    ...
    for (let i = 0; i < ref_args.length; i++) {
      ...
      if (!arg) {
        this.throwError(
          `Function call argument missing a value for ${arg_ref.datatype.value} ${arg_ref.name} and no default value is available`,
          semanticNode,
          arg
        );
        continue;
      ...
      if (arg_ref && this.getValueDataType(arg_ref) != this.getValueDataType(arg)) {
        ...
        if (arg_ref.datatype.value == 'action') {
          if (!('function_reference' in arg) || !arg.function_reference) {
            this.throwError(
              `Can't pass a function call to ${arg_ref.datatype.value} ${arg_ref.name}`,
              semanticNode,
              arg
            );
          }
          ...
          this.throwError(
            `Can't pass a value with a datatype type of [${argTypeStr}] to ${arg_ref.datatype.value} ${arg_ref.name}`,
            semanticNode,
            arg
          );
        }
        ...
        const argTypeStr = argDataType !== undefined ? argDataType : 'unknown';
        this.throwError(
          `Can't pass a value with a datatype type of [${argTypeStr}] to [no argument]`,
          semanticNode,
          arg
        );
      }
  ...
  parseStructNode(statement: StructNode): SemanticStructNode {
    ...
    semanticNode.is_global = this.scope?.is_global ?? true;
  ...
  parsePropertyNode(statement: StructPropertyNode): SemanticPropertyNode {
    ...
    if (semanticNode.left && typeof semanticNode.left == 'object') {
      ...
      semanticNode.property_reference = this.getVariableByName(
        `${semanticNode.left.name}.${semanticNode.name}`
      ) as SemanticStructPropertyNode;
  ...
  parseVariableReferenceNode(statement: VariableReferenceNode): SemanticVariableReferenceNode {
    ...
    if (!varRef) {
      this.throwError(
        `Tried to access a variable name [${statement.name}] that is not in this scope.`,
        semanticNode,
        semanticNode
      );
    }
  ...
  parseVariableNode(statement: VariableNode): SemanticVariableNode {
    ...
    if (statement.declare && statement.is_const) {
      if (
        !this.isValueLiteral(statement.value) ||
        (this.getValueDataType(statement.value) != 'int' &&
          this.getValueDataType(statement.value) != 'float' &&
          this.getValueDataType(statement.value) != 'string')
      ) {
        this.throwError(
          'Variables with declared with the keyword [const] must have a literal value of type [int], [float], or [string]',
          statement,
          statement.value
        );
      }
    ...
    if (semanticNode.struct) {
      ...
      if (!structVar) {
        this.throwError(
          `Tried to access a variable [${semanticNode.struct}] with a type of [struct] that is not in this scope.`,
          semanticNode,
          semanticNode
        );
        return semanticNode;
      }
      if (structVar.type === 'struct' || structVar.type === 'variable') {
      ...
      if (statement.declare) {
        if (structVar.type == 'struct') {
          ...
          this.throwError(
            `Tried to access a struct [${semanticNode.struct}], but the returned type was [${structVar.type}].`,
            semanticNode,
            structVar
          );
        }
      } else {
        const dt = (structVar as { datatype?: { value: string } }).datatype;
        if (dt?.value == 'struct') {
          const sr = (structVar as SemanticVariableNode).struct_reference;
          if (sr) {
            if (sr.type == 'struct') {
              semanticNode.struct_reference = structVar as SemanticVariableNode;
            } else {
              this.throwError(
                `Tried to access a variable [${semanticNode.struct}] expecting a type of [struct], but the returned type was [${dt.value}].`,
                semanticNode,
                semanticNode
              );
            }
            ...
            this.throwError(
              `Tried to access a variable [${semanticNode.struct}], but struct_reference is undefined.`,
              semanticNode,
              semanticNode
            );
          }
          ...
          this.throwError(
            `Tried to access a variable [${semanticNode.struct}] with a type of [${dt?.value ?? 'unknown'}], but expected type of [struct].`,
            semanticNode,
            structVar
          );
        }
      ...
      if (!semanticNode.declare && !this.isNameInUse(semanticNode.name)) {
        this.throwError(
          `Tried to access a variable name [${semanticNode.name}] that is not in this scope.`,
          semanticNode,
          semanticNode
        );
      }
    ...
    if (semanticNode.declare) {
      ...
      if (semanticNode.datatype.struct) {
        ...
        if (structReference) {
          if (structReference.type === 'struct' || structReference.type === 'variable') {
            semanticNode.struct_reference = structReference;
          }
        } else {
          this.throwError(
            `Tried to access a struct [${semanticNode.datatype.struct}] that is not in this scope.`,
            semanticNode,
            semanticNode
          );
        }
        ...
        if (semanticNode.struct_reference.type == 'struct') {
          for (let i = 0; i < semanticNode.struct_reference.properties.length; i++) {
            ...
            if (property.name == semanticNode.name) {
              ...
              semanticNode.is_global = (semanticNode.variable_reference as { is_global?: boolean })?.is_global ?? false;
            }
          }
    ...
    if (
      semanticNode.value &&
      semanticNode.datatype &&
      semanticNode?.datatype?.value != this.getValueDataType(semanticNode.value)
    ) {
      this.throwError(
        `Can't assign a value with a datatype type of [${this.getValueDataType(semanticNode.value)}] to ${semanticNode.datatype.value} ${semanticNode.name}`,
        semanticNode,
        semanticNode.value
      );
    }
  ...
  parseLiteralNode(statement: LiteralNode): SemanticLiteralNode {
    ...
    if (
      ...
      (statement.value as { type?: string }).type == 'literal'
    ) {
      semanticNode.value = this.parseASTStatement(statement.value);
    }
    return semanticNode;
    ...
    if (!!statement.elements && Array.isArray(statement.elements) && statement.elements.length > 0) {
      ...
      for (let i = 0; i < semanticNode.elements.length; i++) {
        ...
        if (element.type != 'literal') {
          this.throwError(
            `${semanticNode.datatype.value} literal elements must be literal at index ${i}`,
            element,
            element
          );
          continue;
  ...
  parseReturnNode(statement: ReturnNode): SemanticReturnNode {
    ...
    const scopeReturnType = this.scopes
      .slice(0)
      .reverse()
      .find((sc) => typeof sc.returntype !== 'undefined')?.returntype;
    if (scopeReturnType) {
      const valueDataType = this.getValueDataType(semanticNode.value);
      if (scopeReturnType.value == 'void') {
        if (semanticNode.value != null)
          this.throwError(
            `Can't return a value with a datatype type of [${valueDataType}] in a scope that expects a datatype of ${scopeReturnType.value}`,
            semanticNode,
            semanticNode.value
          );
  ...
  parseSwitchNode(statement: SwitchNode): SemanticSwitchNode {
    ...
    if (!!semanticNode.default && typeof semanticNode.default == 'object')
      semanticNode.default = this.parseASTStatement(semanticNode.default);
    return semanticNode;
  }
  ...
  parseCompareNode(statement: CompareNode): SemanticCompareNode {
    ...
    if (semanticNode.operator?.value == '&&') {
      if (!(left_type == 'int' && right_type == 'int')) {
        this.throwError(
          `Can't AND types of [${left_type}] and [${right_type}] together`,
          semanticNode,
          semanticNode.right
        );
      }
      ...
      if (
        ...
        if (
          left_type_unary == right_type_unary &&
          left_type_unary >= 0x10 &&
          left_type_unary <= 0x1f &&
          right_type_unary >= 0x10 &&
          right_type_unary <= 0x1f
        ) {
          //
        } else {
          this.throwError(
            `Can't EQUAL types of [${left_type}] and [${right_type}] together`,
            semanticNode,
            semanticNode.right
          );
        }
      ...
      if (
        ...
        if (
          ...
          this.throwError(
            `Can't NEQUAL types of [${left_type}] and [${right_type}] together`,
            semanticNode,
            semanticNode.right
          );
        }
      ...
      if (!(left_type == 'int' && right_type == 'int') && !(left_type == 'float' && right_type == 'float')) {
        this.throwError(
          `Can't GEQ types of [${left_type}] and [${right_type}] together`,
          semanticNode,
          semanticNode.right
        );
      }
      ...
      if (!(left_type == 'int' && right_type == 'int') && !(left_type == 'float' && right_type == 'float')) {
        this.throwError(
          `Can't GT types of [${left_type}] and [${right_type}] together`,
          semanticNode,
          semanticNode.right
        );
      }
      ...
      if (!(left_type == 'int' && right_type == 'int') && !(left_type == 'float' && right_type == 'float')) {
        this.throwError(
          `Can't LEQ types of [${left_type}] and [${right_type}] together`,
          semanticNode,
          semanticNode.right
        );
      }
      ...
      if (!(left_type == 'int' && right_type == 'int') && !(left_type == 'float' && right_type == 'float')) {
        this.throwError(
          `Can't LT types of [${left_type}] and [${right_type}] together`,
          semanticNode,
          semanticNode.right
        );
      }
      ...
      if (!(left_type == 'int' && right_type == 'int')) {
        this.throwError(
          `Can't Left Shift types of [${left_type}] and [${right_type}] together`,
          semanticNode,
          semanticNode.right
        );
      }
      ...
      if (!(left_type == 'int' && right_type == 'int')) {
        this.throwError(
          `Can't Right Shift types of [${left_type}] and [${right_type}] together`,
          semanticNode,
          semanticNode.right
        );
      }
      ...
      if (!(left_type == 'int' && right_type == 'int')) {
        this.throwError(
          `Can't Unsigned Right Shift types of [${left_type}] and [${right_type}] together`,
          semanticNode,
          semanticNode.right
        );
      }
      ...
      if (!(left_type == 'int' && right_type == 'int')) {
        this.throwError(
          `Can't Modulus types of [${left_type}] and [${right_type}] together`,
          semanticNode,
          semanticNode.right
        );
      }
      ...
      if (!(left_type == 'int' && right_type == 'int')) {
        this.throwError(
          `Can't Exclusive OR types of [${left_type}] and [${right_type}] together`,
          semanticNode,
          semanticNode.right
        );
      }
      ...
      if (!(left_type == 'int' && right_type == 'int')) {
        this.throwError(
          `Can't OR types of [${left_type}] and [${right_type}] together`,
          semanticNode,
          semanticNode.right
        );
      }
      ...
      if (!(left_type == 'int' && right_type == 'int')) {
        this.throwError(
          `Can't Inclusive OR types of [${left_type}] and [${right_type}] together`,
          semanticNode,
          semanticNode.right
        );
      }
  ...
  parseAssignNode(statement: AssignNode): SemanticAssignNode {
    ...
    if (semanticNode.type == 'assign') {
      if (left_type != right_type) {
        this.throwError(
          `Can't assign a value of type [${right_type}] to a variable of type [${left_type}]`,
          semanticNode,
          semanticNode.right
        );
      }
  ...
  parseArithmeticNode(statement: BinaryOpNode): any {
    ...
    if (semanticNode.type == 'add') {
      if (
        ...
        this.throwError(
          `Can't add types of [${left_type}] and [${right_type}] together`,
          semanticNode,
          semanticNode.right
        );
      }
      ...
      if (
        ...
        this.throwError(
          `Can't subtract types of [${left_type}] and [${right_type}] together`,
          semanticNode,
          semanticNode.right
        );
      }
      ...
      if (
        ...
        this.throwError(
          `Can't multiply types of [${left_type}] and [${right_type}] together`,
          semanticNode,
          semanticNode.right
        );
      }
  ...
  parseUnaryNode(statement: UnaryNode): SemanticUnaryNode {
    if (
      !statement ||
      typeof statement !== 'object' ||
      (statement.type !== 'not' && statement.type !== 'neg' && statement.type !== 'comp')
    ) {
  ...
  parseIncDecNode(statement: IncDecNode): SemanticIncDecNode {
    ...
    const valueName =
      statement?.value && 'name' in statement.value ? (statement.value as { name: string }).name : undefined;
    semanticNode.variable_reference = this.getVariableByName(valueName);
    if (semanticNode.variable_reference) {
      semanticNode.datatype =
        ('datatype' in semanticNode.variable_reference ? semanticNode.variable_reference.datatype : undefined) ??
        semanticNode.datatype;
      semanticNode.is_global =
        ('is_global' in semanticNode.variable_reference && semanticNode.variable_reference.is_global) ?? false;
...
class NWScriptScope {
  ...
  getVariable(name = '') {
    ...
    if (isStructProperty) {
      ...
      if (struct) {
        if (struct.type == 'variable') {
          return (struct?.struct_reference as SemanticStructNode)?.properties?.find(
    ...
    return (
      ...
      this.structs.find((v) => v.name == name) ||
      this.arguments.find((v) => v.name == name) ||
      this.constants.find((v) => v.name == name)
    );
  }
