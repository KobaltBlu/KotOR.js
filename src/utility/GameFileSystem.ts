...
export class GameFileSystem {
  ...
  static async open(filepath: string, mode: 'r' | 'w' = 'r'): Promise<any> {
    if (ApplicationProfile.ENV == ApplicationEnvironment.ELECTRON) {
      ...
      if (dirHandle) {
        ...
        throw new Error(
          'Failed to locate file directory: ApplicationProfile.directoryHandle might be undefined or missing'
        );
      }
    }
  }
  ...
  static async read(
    handle: FileSystemFileHandle | number,
    output: Uint8Array,
    offset: number,
    length: number,
    position: number
    ...
    if (ApplicationProfile.ENV == ApplicationEnvironment.ELECTRON) {
      ...
      if (!(handle instanceof FileSystemFileHandle))
        throw new Error('FileSystemFileHandle expected but one was not supplied!');
  ...
  private static async readdir_web(
  ...
  private static async readdir_fs(
    resource_path: string = '',
    opts: IGameFileSystemReadDirOptions = {},
    files: any[] = [],
    depthState?: any
  ) {
  ...
  private static async resolvePathDirectoryHandle(
    filepath: string,
    parent = false
    ...
    if (ApplicationProfile.directoryHandle) {
      ...
      for (let i = 0, len = dirs.length; i < len; i++) {
        ...
        if (!found) {
          throw new Error(
            `Failed to resolve file path directory handle: Filepath: ${filepath} | Current Directory: ${dirs[i]} | Index: ${i}`
          );
        }
      }
      this.directoryCache.set(cacheKey, currentDirHandle);
  ...
  static async showRequestDirectoryDialog() {
    const profileKey = typeof ApplicationProfile.profile?.key === 'string' ? ApplicationProfile.profile.key : undefined;
