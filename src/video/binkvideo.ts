...
export class BinkVideoDecoder {
  ...
  private static readonly residueBlock = new Int16Array(64);
  private static readonly runCoordMap = new Int32Array(64);
  ...
  constructor(width: number, height: number, versionChar: string, hasAlpha = false) {
    ...
    decodePacketToFrame(pkt: Uint8Array): YUVFrame {
      ...
      throw new BinkDecodeError(
        `BinkVideoDecoder: Failed to decode packet (${pkt.length} bytes): ${message}`,
        isTruncated
      );
    }
  }
  ...
  private decodePlane(br: BitReaderLE, frame: YUVFrame, plane_idx: number, isChroma: boolean) {
    ...
    /** Copy 8x8 block from src to dest without allocating subarray views. */
    private copyBlock8(dest: Uint8Array, dstOff: number, stride: number, src: Uint8Array, srcOff: number, srcStride: number): void {
        for (let i = 0; i < 8; i++) {
    ...
    private decodeRunBlock(c: PlaneContext, dst: Uint8Array, dstOff: number, stride: number) {
        ...
        const coord = BinkVideoDecoder.runCoordMap;
        ...
        do {
            ...
            if (c.br.readBit()) {
              ...
              this.copyBlock8(base, dstOff, stride, ref.buf, ref.off, ref.stride);
            }
            break;
          }
  ...
  private decodeResidueAdd(c: PlaneContext, dst: Uint8Array, dstOff: number, stride: number) {
    const block = BinkVideoDecoder.residueBlock;
    block.fill(0);
  ...
  private decodeIntra(c: PlaneContext, dst: Uint8Array, dstOff: number, stride: number) {
    const dct = BinkVideoDecoder.intraDct;
    dct.fill(0);
    dct[0] = c.getIntraDC();
  ...
  private decodeScaledBlock(
    ...
    switch (subType) {
      ...
      case BlockType.PATTERN_BLOCK: {
        ...
        for (let j = 0; j < 8; j++) {
          let v = c.getPatternByte();
          for (let i = 0; i < 8; i++, v >>= 1) ublk[i + j * 8] = v & 1 ? c1 : c0;
        }
        break;
      }
      case BlockType.RAW_BLOCK: {
...
class PlaneContext {
    ...
    private readonly _motion = { xoff: 0, yoff: 0 };
  ...
  constructor(
    ...
    base: Uint8Array,
    stride: number,
    prev?: { data: Uint8Array; stride: number }
  ) {
    this.br = br;
  ...
  private decodeRowBundle(id: number) {
    ...
    switch (id) {
      ...
      case BundleId.SUB_BLOCK_TYPES:
        this.readBlockTypes(b, t);
        break;
      case BundleId.PATTERN:
        this.readPatterns(b, t);
        break;
      case BundleId.COLORS:
        this.readColors(b, t);
        break;
      case BundleId.X_OFF:
      case BundleId.Y_OFF:
        this.readMotion(b, t);
        break;
      case BundleId.INTRA_DC:
        this.readDCs(b, t, 11, 0);
        break;
      case BundleId.INTER_DC:
        this.readDCs(b, t, 11, 1);
        break;
      case BundleId.RUN:
        this.readRuns(b, t);
        break;
  ...
  private readRuns(b: Bundle, t: number) {
    ...
    if (this.br.readBit()) {
      const v = this.br.readBits(4) & 0xf;
      b.data.fill(v, b.cur_dec, end);
      b.cur_dec = end;
      return;
    }
  ...
  private readMotion(b: Bundle, t: number) {
    const end = b.cur_dec + t;
    if (this.br.readBit()) {
      let v = this.br.readBits(4) & 0xf;
      if (v) {
        const sign = this.br.readBit() ? -1 : 0;
        v = (v ^ sign) - sign;
      }
  ...
  private readBlockTypes(b: Bundle, t: number) {
    ...
    while (b.cur_dec < end) {
      ...
      if (v < 12) {
        ...
        const run = rleLens[v - 12];
        for (let i = 0; i < run && b.cur_dec < end; i++) b.data[b.cur_dec++] = last;
      }
    }
  }
  ...
  private readDCs(b: Bundle, t: number, startBits: number, hasSign: number) {
    ...
    // Inline writeInt16
    const iv = (v << 16) >> 16;
    b.data[b.cur_dec] = iv & 0xff;
    b.data[b.cur_dec + 1] = (iv >> 8) & 0xff;
    b.cur_dec += 2;
    len--;
  ...
  getBlockType(): number {
    return this.bundle[BundleId.BLOCK_TYPES].data[this.bundle[BundleId.BLOCK_TYPES].cur_ptr++];
  }
  getSubBlockType(): number {
    return this.bundle[BundleId.SUB_BLOCK_TYPES].data[this.bundle[BundleId.SUB_BLOCK_TYPES].cur_ptr++];
  }
  getColor(): number {
    return this.bundle[BundleId.COLORS].data[this.bundle[BundleId.COLORS].cur_ptr++];
  }
  getRawColor(): number {
    return this.bundle[BundleId.COLORS].data[this.bundle[BundleId.COLORS].cur_ptr++];
  }
  getPatternByte(): number {
    return this.bundle[BundleId.PATTERN].data[this.bundle[BundleId.PATTERN].cur_ptr++];
  }
  getRun(): number {
    return this.bundle[BundleId.RUN].data[this.bundle[BundleId.RUN].cur_ptr++];
  }
  getIntraDC(): number {
    const buf = this.bundle[BundleId.INTRA_DC].data,
      off = this.bundle[BundleId.INTRA_DC].cur_ptr;
    // Inline readInt16
  ...
  getInterDC(): number {
    const buf = this.bundle[BundleId.INTER_DC].data,
      off = this.bundle[BundleId.INTER_DC].cur_ptr;
    // Inline readInt16
    const v = buf[off] | (buf[off + 1] << 8);
  ...
  getMotion(): { xoff: number; yoff: number } {
    this._motion.xoff = (this.bundle[BundleId.X_OFF].data[this.bundle[BundleId.X_OFF].cur_ptr++] << 24) >> 24;
    this._motion.yoff = (this.bundle[BundleId.Y_OFF].data[this.bundle[BundleId.Y_OFF].cur_ptr++] << 24) >> 24;
    return this._motion;
  }
...
// Reusable buffers for readDctCoeffs / readResidue (never called concurrently)
const _coefList128 = new Int32Array(128);
const _modeList128 = new Int32Array(128);
const _dctCoefIdx64 = new Int32Array(64);
const _residueNzCoeff64 = new Int32Array(64);
let _residueNzCount = 0;

function readDctCoeffs(
  c: PlaneContext,
  br: BitReaderLE,
  block: Int32Array,
  q: number
): { quantIdx: number; coefIdx: Int32Array; coefCount: number } {
  const coef_list = _coefList128;
  const mode_list = _modeList128;
  const coefIdx = _dctCoefIdx64;
  let list_start = 64,
    list_end = 64;
  let coefCount = 0;
  ...
  function pushCoef(idx: number, val: number) {
    block[scan[idx]] = val | 0;
    coefIdx[coefCount++] = idx;
  }
  ...
  for (; bits >= 0; bits--) {
    ...
    while (list_pos < list_end) {
      if (!(mode_list[list_pos] | coef_list[list_pos]) || !br.readBit()) {
        list_pos++;
        continue;
      }
      ...
      switch (mode) {
        case 0:
          ...
          mode_list[list_pos] = 1; // fallthrough
        case 2:
          if (mode === 2) {
...
function unquantize(
  ...
  for (let i = 0; i < coefCount; i++) {
    const idx = coefIdx[i];
    const si = bink_scan[idx];
    block[si] = (block[si] * qt[idx]) >> 11;
  }
}
...
function readResidue(br: BitReaderLE, block: Int16Array, masksCount: number) {
  const coef_list = _coefList128;
  const mode_list = _modeList128;
  const nz_coeff = _residueNzCoeff64;
  _residueNzCount = 0;
  let list_start = 64,
    list_end = 64;
  ...
  for (let mask = 1 << br.readBits(3); mask; mask >>= 1) {
    for (let i = 0; i < _residueNzCount; i++) {
      if (!br.readBit()) continue;
      const idx = nz_coeff[i];
      if (block[idx] < 0) block[idx] -= mask;
      else block[idx] += mask;
      if (--masksCount < 0) return;
    }
    let list_pos = list_start;
    while (list_pos < list_end) {
      if (!(coef_list[list_pos] | mode_list[list_pos]) || !br.readBit()) {
        list_pos++;
        continue;
      }
      let ccoef = coef_list[list_pos];
        ...
        case 2:
          ...
          for (let i = 0; i < 4; i++, ccoef++) {
            if (br.readBit()) {
              ...
              const idx = bink_scan[ccoef];
              const sign = br.readBit() ? -1 : 0;
              block[idx] = (mask ^ sign) - sign;
              nz_coeff[_residueNzCount++] = idx;
              if (--masksCount < 0) return;
            }
          }
          break;
