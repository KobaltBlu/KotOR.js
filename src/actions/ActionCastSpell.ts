  ...
  update(delta: number = 0): ActionStatus {
    ...
    if (this.spell) {
      if (!this.spell.inRange(this.target, this.owner)) {
        ...
        actionMoveToTarget.setParameter(
          4,
          ActionParameterType.DWORD,
          this.target ? this.target.id : ModuleObjectConstant.OBJECT_INVALID
        );
        ...
        if (this.owner.combatRound) {
          ...
          if (!combatRound.roundPaused) {
            ...
            if (combatRound.roundStarted) {
              const spellCastInstance = new SpellCastInstance(
                this.owner,
                combatRound.action.target,
                combatRound.action.spell
              );
