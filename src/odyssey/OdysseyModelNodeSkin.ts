    ...
    if (this.boneMapCount > 0) {
      ...
      if (this.boneQuaternionDefinition.count > 0) {
        this.odysseyModel.mdlReader.seek(
          this.odysseyModel.fileHeader.modelDataOffset + this.boneQuaternionDefinition.offset
        );
      ...
      if (this.bonePositionDefinition.count > 0) {
        this.odysseyModel.mdlReader.seek(
          this.odysseyModel.fileHeader.modelDataOffset + this.bonePositionDefinition.offset
        );
        for (let i = 0; i < this.bonePositionDefinition.count; i++) {
          this.bone_translations[i] = new THREE.Vector3(
            this.odysseyModel.mdlReader.readSingle(),
            this.odysseyModel.mdlReader.readSingle(),
            this.odysseyModel.mdlReader.readSingle()
          );
      ...
      if (this.boneConstantsDefinition.count > 0) {
        this.odysseyModel.mdlReader.seek(
          this.odysseyModel.fileHeader.modelDataOffset + this.boneConstantsDefinition.offset
        );
