...
export class OdysseyWalkMesh {
  ...
  constructor(wokReader?: BinaryReader) {
    ...
    for (let i = 0, len = this.faces.length; i < len; i++) {
      ...
      face.triangle = new THREE.Triangle(this.vertices[face.a], this.vertices[face.b], this.vertices[face.c]);
  ...
  buildBufferedGeometry() {
    ...
    const colors = this.faces
      .map((f) => {
        return [f.color.r, f.color.g, f.color.b, f.color.r, f.color.g, f.color.b, f.color.r, f.color.g, f.color.b];
      })
      .flat();
  ...
  readBinary() {
    if (this.wokReader instanceof BinaryReader) {
      ...
      for (let i = 0; i < this.header.facesCount; i++) this.walkTypes[i] = this.wokReader.readInt32();

      //READ Normals
      this.wokReader.seek(this.header.offsetToNormalizedInvertedNormals);
      for (let i = 0; i < this.header.facesCount; i++) {
        this.faces[i].normal = this.normals[i] = new THREE.Vector3(
          this.wokReader.readSingle(),
          this.wokReader.readSingle(),
          this.wokReader.readSingle()
        );
      ...
      for (let i = 0; i < this.header.aabbCount; i++) this.aabbNodes[i] = this.readAABB();
  ...
  detectFacePerimiterLines(face: OdysseyFace3) {
    ...
    for (let i = 0; i < this.faces.length; i++) {
      ...
      for (let j = 0; j < 3; j++) {
        if (j == 0 && aEdge) {
          if (
            (face.a == adjFace.a && face.b == adjFace.b) ||
            (face.a == adjFace.b && face.b == adjFace.a) ||
            (face.a == adjFace.b && face.b == adjFace.c) ||
            (face.a == adjFace.c && face.b == adjFace.b) ||
            (face.a == adjFace.c && face.b == adjFace.a) ||
            (face.a == adjFace.a && face.b == adjFace.c)
          ) {
            aEdge = false;
          }
        } else if (j == 1 && bEdge) {
          if (
            (face.b == adjFace.a && face.c == adjFace.b) ||
            (face.b == adjFace.b && face.c == adjFace.a) ||
            (face.b == adjFace.b && face.c == adjFace.c) ||
            (face.b == adjFace.c && face.c == adjFace.b) ||
            (face.b == adjFace.c && face.c == adjFace.a) ||
            (face.b == adjFace.a && face.c == adjFace.c)
          ) {
            bEdge = false;
          }
        } else if (j == 2 && cEdge) {
          if (
            (face.c == adjFace.a && face.a == adjFace.b) ||
            (face.c == adjFace.b && face.a == adjFace.a) ||
            (face.c == adjFace.b && face.a == adjFace.c) ||
            (face.c == adjFace.c && face.a == adjFace.b) ||
            (face.c == adjFace.c && face.a == adjFace.a) ||
            (face.c == adjFace.a && face.a == adjFace.c)
          ) {
            cEdge = false;
          }
        }
    ...
    if (this.header.walkMeshType == OdysseyWalkMeshType.AABB) {
      ...
      if (node == undefined) {
        ...
        if (node) {
          ...
          if (node.faceIdx > -1) collisions.push(this.faces[node.faceIdx]);
        }
      } else {
        if (this.containsPointOrBox(node.box, box)) {
          if (node.leftNode != undefined) {
  ...
  static Init() {
    ...
    if (tilecolor2DA) {
      for (let i = 0; i < tilecolor2DA.RowCount; i++) {
        ...
        OdysseyWalkMesh.TILECOLORS.push(TileColor.From2DA(tileColor));
      }
    }
   ...
   */
  private findFaceSharingEdge(
    v1: number,
    v2: number,
    excludeFaceIdx: number
  ): { faceIdx: number; edgeIndex: number } | null {
    const matchEdge = (a: number, b: number) => (a === v1 && b === v2) || (a === v2 && b === v1);
  ...
  rebuild(): void {
    ...
    for (let i = 0; i < this.faces.length; i++) {
      ...
      face.centroid
        .set(0, 0, 0)
        .add(this.vertices[face.a])
        .add(this.vertices[face.b])
        .add(this.vertices[face.c])
        .divideScalar(3);
    ...
    for (let i = 0; i < this.walkableFaces.length; i++) {
      ...
      face.adjacentWalkableFaces.a =
        face.adjacent[0] >= 0 ? this.faces[face.adjacent[0]] : face.adjacentWalkableFaces.a;
      face.adjacentWalkableFaces.b =
        face.adjacent[1] >= 0 ? this.faces[face.adjacent[1]] : face.adjacentWalkableFaces.b;
      face.adjacentWalkableFaces.c =
        face.adjacent[2] >= 0 ? this.faces[face.adjacent[2]] : face.adjacentWalkableFaces.c;
      if (
        face.adjacentWalkableFaces.a instanceof WalkmeshEdge ||
        face.adjacentWalkableFaces.b instanceof WalkmeshEdge ||
        face.adjacentWalkableFaces.c instanceof WalkmeshEdge
  ...
  private buildAABBTree(): void {
    ...
    const getFaceBox = (faceIdx: number): THREE.Box3 => {
      ...
      box.setFromPoints([this.vertices[face.a].clone(), this.vertices[face.b].clone(), this.vertices[face.c].clone()]);
      return box;
    };
