...
function emitMeshAscii(mesh: OdysseyModelNodeMesh, model: OdysseyModel, nl: string): string {
  ...
  s +=
    nl +
    '  diffuse ' +
    prepareFloat(mesh.diffuse.r) +
    ' ' +
    prepareFloat(mesh.diffuse.g) +
    ' ' +
    prepareFloat(mesh.diffuse.b);
  s +=
    nl +
    '  ambient ' +
    prepareFloat(mesh.ambient.r) +
    ' ' +
    prepareFloat(mesh.ambient.g) +
    ' ' +
    prepareFloat(mesh.ambient.b);
...
function emitKeyedController(
  ...
  const colLow = col & 15;
  s += nl + '      ' + name + (bezierFlag && !opts.bezierToLinear ? 'bezier' : '') + 'key';
...
function emitAnimationNode(
  model: OdysseyModel,
  anode: OdysseyModelAnimationNode,
  nl: string,
  opts: ExportOdysseyModelAsciiOptions
): string {
  let s = '';
...
export function exportOdysseyModelAscii(model: OdysseyModel, options: ExportOdysseyModelAsciiOptions = {}): string {
  ...
  out +=
    nl +
    '  bmin ' +
    prepareFloat(model.modelHeader.boundingMinX) +
    ' ' +
    prepareFloat(model.modelHeader.boundingMinY) +
    ' ' +
    prepareFloat(model.modelHeader.boundingMinZ);
  out +=
    nl +
    '  bmax ' +
    prepareFloat(model.modelHeader.boundingMaxX) +
    ' ' +
    prepareFloat(model.modelHeader.boundingMaxY) +
    ' ' +
    prepareFloat(model.modelHeader.boundingMaxZ);
  out += nl + '  radius ' + prepareFloat(model.modelHeader.radius);
  if (model.asciiLayoutPosition) {
    const lp = model.asciiLayoutPosition;
    out += nl + '  layoutposition ' + prepareFloat(lp.x) + ' ' + prepareFloat(lp.y) + ' ' + prepareFloat(lp.z);
  }
