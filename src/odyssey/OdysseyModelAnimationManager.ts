...
export class OdysseyModelAnimationManager {
  ...
  updateAnimationNode(
    ...
    node: OdysseyModelAnimationNode,
    state: OdysseyModelAnimationManagerState,
    canTween: boolean = false
  ) {
    if (typeof state == 'undefined') {
    ...
    for (const c of node.controllers) {
      ...
      if (controller.type == OdysseyModelControllerType.Position) {
        const tweenFL =
          Math.min(this.currentAnimation.transition, this.transElapsed) / this.currentAnimation.transition;
