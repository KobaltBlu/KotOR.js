  ...
  setFrame(manager: OdysseyModelAnimationManager, anim: OdysseyModelAnimation, data: IOdysseyControllerFrameGeneric) {
    if (manager.modelNode.userData.mesh) {
      if (manager.modelNode.userData.mesh.material instanceof THREE.ShaderMaterial) {
        manager.modelNode.userData.mesh.material.uniforms.selfIllumColor.value.setRGB(data.x, data.y, data.z);
        manager.modelNode.userData.mesh.material.defines.SELFILLUMCOLOR = '';
      } else {
        manager.modelNode.userData.mesh.material.emissive.setRGB(data.x, data.y, data.z);
      }
    }
  }
  ...
  animate(
    ...
    if (manager.modelNode.userData.mesh) {
      if (manager.modelNode.userData.mesh.material instanceof THREE.ShaderMaterial) {
        ...
        manager.modelNode.userData.mesh.material.emissive.setRGB(lerpIllumColorR, lerpIllumColorG, lerpIllumColorB);
      }
