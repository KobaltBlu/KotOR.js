  ...
  setFrame(manager: OdysseyModelAnimationManager, anim: OdysseyModelAnimation, data: IOdysseyControllerFrameGeneric) {
    if (manager.modelNode && manager.model) {
      ...
      if (typeof manager.modelNode.controllers.get(OdysseyModelControllerType.Scale) != 'undefined') {
        _offsetScale =
          manager.modelNode.controllers.get(OdysseyModelControllerType.Scale).data[0].value || 0.000000000001; // 0 scale causes warnings; used when uncommenting line below
      }
