  ...
  async loadStuntActor(actor: IDLGStuntActor) {
    return new Promise<void>((resolve, reject) => {
      ...
      if (actor.participant == 'PLAYER') {
          ...
          .then((actorModel: OdysseyModel) => {
            OdysseyModel3D.FromMDL(actorModel)
              .then((actorSuperModel: OdysseyModel3D) => {
                ...
                if (this.isAnimatedCutscene) GameState.PartyManager.party[0].setFacing(0, true);

                if (this.unequipItems) GameState.PartyManager.party[0].UnequipItems();

                if (this.unequipHeadItem) GameState.PartyManager.party[0].UnequipHeadItem();
        ...
        if (this.isAnimatedCutscene) this.owner.setFacing(0, true);
            ...
            .then((actorModel: OdysseyModel) => {
              OdysseyModel3D.FromMDL(actorModel)
                .then((actorSuperModel: OdysseyModel3D) => {
                  ...
                  if (this.isAnimatedCutscene) creature.setFacing(0, true);
