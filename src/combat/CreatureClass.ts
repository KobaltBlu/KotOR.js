...
export class CreatureClass {
  ...
  constructor(id = -1) {
    ...
    this.level = 0;
    this.spells = [];
    if (id >= 0) this.apply2DA(GameState.TwoDAManager.datatables.get('classes').rows[this.id]);
  }
      ...
      if (known_struct) {
        ...
        for (let i = 0; i < known_spell_structs.length; i++) {
          ...
          if (known_spell_struct.hasField('Spell')) {
            spell = new GameState.TalentSpell(known_spell_struct.getFieldByLabel('Spell').getValue());
          }
  ...
  apply2DA(row: any) {
    ...
    if (row.hasOwnProperty('hitdie')) this.hitdie = TwoDAObject.normalizeValue(row.hitdie, 'number', 8);
    ...
    if (row.hasOwnProperty('featstable')) this.featstable = TwoDAObject.normalizeValue(row.featstable, 'string', 'SOL');
    ...
    if (row.hasOwnProperty('str')) this.str = TwoDAObject.normalizeValue(row.str, 'number', 10);

    if (row.hasOwnProperty('dex')) this.dex = TwoDAObject.normalizeValue(row.dex, 'number', 10);

    if (row.hasOwnProperty('con')) this.con = TwoDAObject.normalizeValue(row.con, 'number', 10);

    if (row.hasOwnProperty('wis')) this.wis = TwoDAObject.normalizeValue(row.wis, 'number', 10);

    if (row.hasOwnProperty('int')) this.int = TwoDAObject.normalizeValue(row.int, 'number', 10);

    if (row.hasOwnProperty('cha')) this.cha = TwoDAObject.normalizeValue(row.cha, 'number', 10);
    ...
    if (row.hasOwnProperty('forcedie')) this.forcedie = TwoDAObject.normalizeValue(row.forcedie, 'number', 0);

    if (row.hasOwnProperty('armorclasscolumn'))
      this.armorclasscolumn = TwoDAObject.normalizeValue(row.armorclasscolumn, 'string', 'SOL');

    if (row.hasOwnProperty('featgain')) this.featgain = TwoDAObject.normalizeValue(row.featgain, 'string', 'SOL');
