...
import { getCombatMessageText, COMBAT_MESSAGE_DURATION_MS } from "./CombatMessageTLK";
...
export class CombatRound {
  ...
  /**
   * Log attack result to the in-game combat message label.
   * Reva: CSWPartyMemberData::SetCombatMessage only updates GUI when creature is player (GetPlayerCreatureId).
   * So we only log for the party leader's attacks.
   */
  private logAttackToCombatMenu(creature: ModuleCreature, combatAction: CombatRoundAction) {
    const party = GameState.PartyManager?.party;
    if (!party?.length || creature !== party[0]) return;
  ...
  calculateWeaponAttack(creature: ModuleCreature, weapon: ModuleItem | undefined = undefined, weaponSlot: ModuleCreatureArmorSlot, combatAction: CombatRoundAction) {
    ...
    this.logAttackToCombatMenu(creature, combatAction);
