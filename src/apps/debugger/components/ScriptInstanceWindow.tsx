...
const InstructionOffset = (props: { instance: KotOR.NWScriptInstance; instruction: KotOR.NWScriptInstruction }) => {
  ...
  return instruction.offset ? (
    <>
      <a href={`#${address}`} className="instruction-offset" onClick={onClick}>
        {address}
      </a>
    </>
  ) : (
...
const InstructionNode = (props: { instance: KotOR.NWScriptInstance; instruction: KotOR.NWScriptInstruction }) => {
  ...
  return (
    <li
      ...
      id={instruction.address_hex}
    >
      <div
        className="breakpoint-clicker"
        title="Toggle breakpoint"
        onClick={() => {
          callToggleBreakpoint();
        }}
      ></div>
      <span className="instruction-address">{instruction.address_hex}</span> -{' '}
      <span className={`instruction-codeName ${instruction.codeName}`}>{instruction.codeName.padEnd(8, '\u00a0')}</span>{' '}
      -{' '}
      {instruction.action ? (
        <>
      ...
      {instruction.type != undefined ? (
        <>
          {instruction.code == OP_CONST || instruction.code == OP_RSADD ? (
