...
export const TabScriptErrorLog = function (props: any) {
  ...
  const getSeverityMeta = (severity?: number) => {
    return (
      severityConfig[severity || monacoEditor.MarkerSeverity.Info] || severityConfig[monacoEditor.MarkerSeverity.Info]
    );
  };
              ...
              >
                ...
                <span className="error-list__body">
                  ...
                  <span className="error-list__meta">
                    ...
                    <span className="error-list__meta-item error-list__location">
                      Ln {error.startLineNumber || 0}, Col {error.startColumn || 0}
                    </span>
                    {code ? <span className="error-list__meta-item error-list__code">{code}</span> : null}
                    {error.source ? (
                      <span className="error-list__meta-item error-list__source">{error.source}</span>
                    ) : null}
                  </span>
                </span>
              </button>
