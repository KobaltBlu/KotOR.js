...
export const TabUTMEditor = function (props: BaseTabProps) {
            ...
            <tbody>
              ...
              >
                <input
                  ...
                  maxLength={32}
                  value={tag}
                  onChange={onUpdateCExoStringField(setTag, 'tag')}
                />
              </FormField>
              <FormField
                label="ResRef"
              ...
              >
                <select
                  value={buySellFlag}
                  onChange={(e) => {
                    setBuySellFlag(Number(e.target.value));
                    tab.store.buySellFlag = Number(e.target.value);
                    tab.updateFile();
                  }}
                >
              ...
              <FormField label="Mark Down" info="Percentage discount when selling items to the store (0-100).">
                <input
                  type="number"
                  min="0"
                  max="100"
                  value={markDown}
                  onChange={onUpdateNumberField(setMarkDown, 'markDown')}
                />
              </FormField>
              <FormField label="Mark Up" info="Percentage markup when buying items from the store (0-100).">
                <input
                  type="number"
                  min="0"
                  max="100"
                  value={markUp}
                  onChange={onUpdateNumberField(setMarkUp, 'markUp')}
                />
              </FormField>
              <FormField label="On Open Store" info="Script that runs when the store is opened by a player.">
                <input
                  type="text"
                  placeholder="Enter script name"
                  maxLength={16}
                  value={onOpenStore}
                  onChange={onUpdateResRefField(setOnOpenStore, 'onOpenStore')}
                />
              </FormField>
            </tbody>
          </table>
                ...
                <td colSpan={2}>
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', gap: '0.4rem' }}>
                      <button onClick={onOpenInventoryBrowser} className="btn btn-sm btn-outline-secondary">
                        Inventory Browser
                      </button>
                      <button onClick={onAddItem} className="btn btn-sm btn-primary">
  ...
  return <SubTabHost tabs={tabs} defaultTab="basic" />;
};
