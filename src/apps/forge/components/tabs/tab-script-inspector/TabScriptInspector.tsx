...
export const TabScriptInspector = function (props: any) {
  ...
  const onCopyAssemblyToClipboard = async () => {
    try {
      const assemblyText = instructions.map((instruction) => instruction.toAssemblyString()).join('\n');
  ...
  return (
    ...
    >
      ...
      >
        <table
          className="table table-stripped text-light"
          style={{ width: `auto`, fontFamily: `'Courier New', monospace`, whiteSpace: `pre` }}
        >
          <thead>
            <tr>
              <th>Address</th>
          ...
          <tbody>
            {instructions.map((instruction) => {
              const address = (
                '000000000' + (parseInt(instruction.address as any, 16) + offset).toString(16).toUpperCase()
              ).substr(-8);
