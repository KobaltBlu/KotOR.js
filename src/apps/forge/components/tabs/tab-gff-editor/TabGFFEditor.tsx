...
export const TabGFFEditor = function (props: BaseTabProps) {
  ...
  return (
    <>
      <div
        id="gffContainer"
        style={{ position: 'relative', overflow: 'auto', height: '100%', width: '50%', float: 'left' }}
      >
        <ForgeTreeView>
          {gff ? (
            <GFFStructElement struct={gff.RootNode} key={gff.RootNode.uuid} open={true} tab={tab} depth={0} />
          ) : (
            <></>
          )}
        </ForgeTreeView>
      ...
      <div
        id="gffProperties"
        className="container"
        style={{ position: 'relative', overflow: 'auto', height: '100%', width: '50%', padding: '10px', float: 'left' }}
      >
        {selectedNode ? (
...
const GFFStructElement = memo(function GFFStructElement(props: any) {
  ...
  const childNodes = useMemo(() => {
    ...
    const fieldNodes = struct
      .getFields()
      .map((field: KotOR.GFFField) => (
        <GFFFieldElement field={field} key={field.uuid} tab={props.tab} depth={depth + 1} />
      ));
...
const GFFFieldElement = memo(function GFFFieldElement(props: any) {
  ...
  const childNodes = useMemo(() => {
    ...
    const structNodes = field
      .getChildStructs()
      .map((struct: KotOR.GFFStruct) => (
        <GFFStructElement struct={struct} key={struct.uuid} tab={props.tab} depth={depth + 1} />
      ));
    ...
    switch (node.getType()) {
      ...
      case KotOR.GFFDataType.CEXOSTRING:
        return (
          <fieldset>
            <legend>
              [{KotOR.GFFDataType[node.getType()]}] - {node.getLabel()}
            </legend>
            <InputGroup>
