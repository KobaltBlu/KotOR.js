      ...
      ...[
        { id: 'separator-1', separator: true },
        {
          ...
          onClick: async () => {
            ...
            if (
              resource.resType == KotOR.ResourceTypes['erf'] ||
              resource.resType == KotOR.ResourceTypes['mod'] ||
              resource.resType == KotOR.ResourceTypes['sav']
            ) {
              const erf =
                resource.resType == KotOR.ResourceTypes['rim']
                  ? new KotOR.RIMObject(exportBufferArchive)
                  : new KotOR.ERFObject(exportBufferArchive);
              await erf.load();
              if (KotOR.ApplicationProfile.ENV == KotOR.ApplicationEnvironment.ELECTRON) {
                const savePath = await dialog.showSaveDialog({
                ...
                for (const key of resources) {
                  ...
                  fs.writeFile(
                    savePath.filePath + '/' + key.resRef + '.' + KotOR.ResourceTypes.getKeyByValue(key.resType),
                    exportBuffer,
                    () => {
                ...
                for (const key of resources) {
                  ...
                  const fileHandle = await directoryHandle.getFileHandle(
                    key.resRef + '.' + KotOR.ResourceTypes.getKeyByValue(key.resType),
                    { create: true }
                  );
                  if (!fileHandle) {
