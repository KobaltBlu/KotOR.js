...
export const DLGTreeView: React.FC<DLGTreeViewProps> = ({
  ...
  const handleNodeClick = useCallback((node: DLGTreeNode) => {
    model.selectNode(node.id);
  }, [model]);

  const handleNodeDoubleClick = useCallback((node: DLGTreeNode) => {
  ...
  const handleToggleExpand = useCallback((node: DLGTreeNode, event: React.MouseEvent) => {
  ...
  const handleDragStart = useCallback((node: DLGTreeNode, event: React.DragEvent) => {
    ...
    event.dataTransfer.setData('application/dlg-node', JSON.stringify({
      nodeId: node.id,
  ...
  const handleDragOver = useCallback((node: DLGTreeNode, event: React.DragEvent) => {
  ...
  const handleDrop = useCallback((node: DLGTreeNode, event: React.DragEvent) => {
    event.preventDefault();
  ...
  const handleContextMenu = useCallback((node: DLGTreeNode, event: React.MouseEvent) => {
    event.preventDefault();
  ...
  const renderNode = useCallback((node: DLGTreeNode, depth: number = 0): React.ReactNode => {
    ...
    return (
      <div key={node.id} className="dlg-tree-node-container">
        ...
        {isExpanded && node.childrenLoaded && node.children.length > 0 && (
          <div className="dlg-tree-node-children">
