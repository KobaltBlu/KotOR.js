...
export class ForgeEncounter extends ForgeGameObject {
    ...
    if (root.hasField('CreatureList')) {
      ...
      this.creatureList = creatureListField.getChildStructs().map((struct) => {
        return {
          ...
          guaranteedCount: struct.hasField('GuaranteedCount')
            ? struct.getFieldByLabel('GuaranteedCount').getValue() || 0
            : undefined,
  ...
  getGITInstance(): KotOR.GFFStruct {
    ...
    for (let i = 0, len = this.spawnPointList.length; i < len; i++) {
      ...
      spawnPointStruct.addField(
        new KotOR.GFFField(KotOR.GFFDataType.FLOAT, 'Orientation', this.spawnPointList[i].orientation)
      );
