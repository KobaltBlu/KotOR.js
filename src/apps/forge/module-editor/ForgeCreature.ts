...
export class ForgeCreature extends ForgeGameObject {
    ...
    try {
      ...
      switch (slot) {
        case KotOR.ModuleCreatureArmorSlot.HEAD:
          ...
          itemTemplate = new KotOR.GFFObject(await KotOR.ResourceLoader.loadResource(utiResId, this.slotHead));
          slotKey = 'headItem';
          break;
        case KotOR.ModuleCreatureArmorSlot.ARMOR:
          if (!this.slotArmor) return;
          itemTemplate = new KotOR.GFFObject(await KotOR.ResourceLoader.loadResource(utiResId, this.slotArmor));
          slotKey = 'armorItem';
          break;
        case KotOR.ModuleCreatureArmorSlot.LEFTHAND:
          if (!this.slotLeftHand) return;
          itemTemplate = new KotOR.GFFObject(await KotOR.ResourceLoader.loadResource(utiResId, this.slotLeftHand));
          slotKey = 'leftHandItem';
          break;
        case KotOR.ModuleCreatureArmorSlot.RIGHTHAND:
          if (!this.slotRightHand) return;
          itemTemplate = new KotOR.GFFObject(await KotOR.ResourceLoader.loadResource(utiResId, this.slotRightHand));
  ...
  async loadBody(): Promise<KotOR.OdysseyModel3D> {
    ...
    const { model: bodyModel, texture: bodyTexture } = appearance.getBodyModelInfo(
      bodyVariation || '',
      textureVariation || 1
    );
  ...
  loadFromBlueprint() {
      ...
      this.classList =
        root
          ...
          .map((struct) => {
            ...
            const knownList0 = struct
              .getFieldByLabel('KnownList0')
              .getChildStructs()
              .map((struct) => {
    ...
    if (root.hasField('ItemList')) {
      this.itemList =
        root
          .getFieldByLabel('ItemList')
          .getChildStructs()
          .map((struct) => {
            const resref: string = struct.hasField('InventoryRes')
              ? struct.getFieldByLabel('InventoryRes').getValue() || ''
              : '';
            const droppable: boolean = struct.hasField('Dropable')
              ? !!struct.getFieldByLabel('Dropable').getValue()
              : false;
            const infinite: boolean = struct.hasField('Infinite')
              ? !!struct.getFieldByLabel('Infinite').getValue()
              : false;
    ...
    if (root.hasField('SpecAbilityList')) {
      this.specAbilityList =
        root
          .getFieldByLabel('SpecAbilityList')
          .getChildStructs()
          .map((struct) => {
  ...
  exportToBlueprint(): KotOR.GFFObject {
    ...
    if (classList) {
      this.classList.forEach((classEntry) => {
        ...
        classEntry.knownList0.forEach((knownSpellEntry) => {
          ...
          knownSpellStruct.addField(
            new KotOR.GFFField(KotOR.GFFDataType.BYTE, 'SpellMetaMagic', knownSpellEntry.spellMetaMagic)
          );
          knownSpellStruct.addField(
            new KotOR.GFFField(KotOR.GFFDataType.BYTE, 'SpellFlags', knownSpellEntry.spellFlags)
          );
    ...
    if (specAbilityList) {
      this.specAbilityList.forEach((specAbility) => {
        ...
        specAbilityStruct.addField(
          new KotOR.GFFField(KotOR.GFFDataType.BYTE, 'SpellCasterLevel', specAbility.spellCasterLevel)
        );
  ...
  getGITInstance(): KotOR.GFFStruct {
    ...
    instance.addField(
      new KotOR.GFFField(KotOR.GFFDataType.FLOAT, 'XOrientation', Math.cos(this.container.rotation.z + Math.PI / 2))
    );
    instance.addField(new KotOR.GFFField(KotOR.GFFDataType.FLOAT, 'XPosition', this.container.position.x));
    instance.addField(
      new KotOR.GFFField(KotOR.GFFDataType.FLOAT, 'YOrientation', Math.sin(this.container.rotation.z + Math.PI / 2))
    );
