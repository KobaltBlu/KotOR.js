...
export class UI3DRenderer extends EventListenerModel {
  ...
  onMouseDown(event: MouseEvent) {
    ...
    if (KotOR.Mouse.ButtonState == KotOR.MouseState.LEFT && !this.transformControlsDragging) {
      ...
      if (intersects.length) {
        ...
        const isVertexHelper =
          closestIntersection.object instanceof THREE.Mesh &&
  ...
  toggleVisibilityByType(type: ObjectType) {
    switch (type) {
      ...
      case 'walkmesh':
        this.group.rooms.children.forEach((child) => {
          ((child as KotOR.OdysseyModel3D).wok.mesh.material as THREE.Material).visible =
            !this.visibilityState[ObjectType.WALKMESH];
        });
        break;
      case 'creature':
  ...
  selectObject(object: THREE.Object3D | undefined) {
    ...
    const isGeometry =
      ...
      (nodeType & KotOR.OdysseyModelNodeType.Mesh) == KotOR.OdysseyModelNodeType.Mesh;
  ...
  private buildCamera() {
    ...
    this.camera.position.set(0.1, 5, 1); // offset the camera a bit
