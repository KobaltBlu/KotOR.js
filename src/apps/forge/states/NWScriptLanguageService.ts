...
function formatNWScript(
  code: string,
  options: monacoEditor.languages.FormattingOptions = { tabSize: 2, insertSpaces: true }
): string {
  try {
...
export class NWScriptLanguageService {
  ...
  static initNWScriptLanguage() {
    ...
    const tokenConfig: monacoEditor.languages.IMonarchLanguage = {
      ...
      parenFollows: ['if', 'for', 'while', 'switch'],
            ...
            try {
              ...
              if (Array.isArray(l_variables)) {
                for (let i = 0; i < l_variables.length; i++) {
                  ...
                  const kind = l_variable.is_const
      ...
      provideHover: function (
        ...
        if (wordObject) {
          ...
          if (action) {
            ...
            const function_definition = ForgeState.nwScriptParser.engine_actions.find(
              (a) => a.name === wordObject.word || a.name === action[1].name || a.name === action[0]
            );
            if (!function_definition) {
              ...
              return {
                contents: [{ value: `**nwscript.nss**` }, { value: hoverContent }],
              };
            }
          ...
          const parser = (ForgeState.tabManager.currentTab as any).nwScriptParser;
          if (parser) {
            const structPropertyMatches = model
              .getValue()
              .matchAll(
                new RegExp(
            ...
            if (l_function) {
              ...
              return {
                contents: [{ value: '**SOURCE**' }, { value: hoverContent }],
              };
            }
          }
        }
        return {
          range: new monacoEditor.Range(1, 1, model.getLineCount(), model.getLineMaxColumn(model.getLineCount())),
        };
      },
    } as any);
            ...
            if (statement.type === 'function') {
              ...
              for (const nameInfo of variable.names) {
                symbols.push({
                  ...
                  kind: variable.is_const
                    ? monacoEditor.languages.SymbolKind.Constant
                    : monacoEditor.languages.SymbolKind.Variable,
                  range: {
      ...
      provideDefinition: function (
        ...
        try {
            ...
            if (resref) {
              ...
              if (targetTab && targetTab.constructor.name === 'TabTextEditorState') {
                ...
                if (key) {
                  ...
                  KotOR.KEYManager.Key.getFileBuffer(key)
                    .then((buffer: Uint8Array) => {
                      if (buffer) {
                        ...
                        const findAndScroll = () => {
                          const newTab = ForgeState.tabManager.tabs.find(
                            (tab) => tab.file && tab.file.resref === resref && tab.file.ext === 'nss'
                          ) as any;
          ...
          if (nw_constant && nw_constant.source) {
            ...
            const nwscriptTab = ForgeState.tabManager.tabs.find(
              (tab) => tab.file && tab.file.resref === 'nwscript' && tab.file.ext === 'nss'
            ) as any;
          ...
          if (nw_action && nw_action.source) {
            ...
            if (nwscriptTab) {
              ...
              if (ForgeState.nwscript_nss) {
                ...
                const findAndScroll = () => {
                  const newTab = ForgeState.tabManager.tabs.find(
                    (tab: TabState) => tab.file && tab.file.resref === 'nwscript' && tab.file.ext === 'nss'
                  ) as any;
