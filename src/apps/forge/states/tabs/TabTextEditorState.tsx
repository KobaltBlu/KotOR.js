    ...
    if (langId === 'nwscript') {
      this.resolveIncludes(this.code, this.resolvedIncludes).then((resolvedIncludes) => {
        ...
        try {
          ...
          const localFunctions = (this.nwScriptParser.program?.functions || []).map(
            (f: SemanticFunctionNode) => f.name
          );
          ...
          if (this.editor)
            this.monaco.editor.setModelMarkers(
              this.editor.getModel() as monacoEditor.editor.ITextModel,
              'nwscript',
              markers
            );
        } catch (e) {
    ...
    if (!ForgeState.nwScriptParser.errors.length) {
      ...
      for (let i = 0; i < ForgeState.nwScriptParser.errors.length; i++) {
        ...
        console.error(
          `Error ${i}:`,
          error.message,
          error.offender?.source?.first_line,
          error.offender?.source?.first_column
        );
      }
    }
