...
import {
  ...
  ObjectType,
  UI3DRenderer,
  UI3DRendererEventListenerTypes,
} from '@/apps/forge/UI3DRenderer';
...
export class TabPTHEditorState extends TabState {
    ...
    for (let i = 0; i < this.points.length; i++) {
      ...
      for (let j = 0; j < point.connections.length; j++) {
        ...
        positions.push(
          ...
          point.vector.y,
          point.vector.z || 0,
          connectedPoint.vector.x,
          connectedPoint.vector.y,
          connectedPoint.vector.z || 0
        );
  ...
  updateFile() {
    ...
    for (let i = 0, len = this.points.length; i < len; i++) {
      const point = this.points[i];
      const validConnections = point.connections.filter(
        (destinationPoint) => this.points.indexOf(destinationPoint) >= 0
      );
      ...
      pathPointStruct.addField(new KotOR.GFFField(KotOR.GFFDataType.DWORD, 'Conections', validConnections.length));
      ...
      for (let j = 0; j < validConnections.length; j++) {
        ...
        const destinationPoint = validConnections[j];
