...
export class ResourceLoader {
  ...
  static async InitGlobalCache() {
    ...
    await Promise.all(
      keys.map(async (key) => {
        ...
        scope.get(key.resType).set(key.resRef.toLocaleLowerCase(), buffer);
      })
    );
  }
      ...
      archives.map(async (archive) => {
        if (archive instanceof RIMObject) {
          ...
          for (let i = 0; i < resources.length; i++) {
            ...
            scope.get(resource.resType).set(resource.resRef.toLocaleLowerCase(), buffer);
          }
  ...
  static setCache(type: CacheScope, resId: number, resRef: string, buffer: Uint8Array) {
    ...
    if (typeof ResourceLoader.cache[resId] === 'undefined') ResourceLoader.cache[resId] = {};
  ...
  static async searchModuleArchives(resId: number, resRef = ''): Promise<Uint8Array> {
    ...
    for (let i = 0; i < archiveCount; i++) {
      ...
      if (archive instanceof RIMObject) {
        if (!archive.hasResource(resRef, resId)) {
          continue;
        }
  ...
  static async searchModules(resId: number, resRef: string): Promise<Uint8Array> {
    ...
    for (let i = 0; i < rimCount; i++) {
      const rim = rims[i];
      if (!rim || !rim.hasResource(resRef, resId)) {
        continue;
      }
