...
export class GUIProgressBar extends GUIControl {
  ...
  constructor(menu: GameMenu, control: GFFStruct, parent: GUIControl, scale: boolean = false) {
    ...
    this.progress.mesh = new THREE.Mesh(this.progress.geometry, [
      this.progress.edge_material,
      this.progress.corner_material,
    ]);
    ...
    if (this.control) {
      ...
      if (this.hasProgress) {
        ...
        if (progress) {
          ...
          this.progress.inneroffset = this.progress.inneroffsety =
            progress.getFieldByLabel('INNEROFFSET')?.getValue() || 0;
    ...
    if (this.progress.edge != '') {
      TextureLoader.enQueue(
        ...
        (texture: OdysseyTexture) => {
          if (!texture) console.log('initTextures', this.progress.edge, texture);
    ...
    if (this.progress.corner != '') {
      TextureLoader.enQueue(
        this.progress.corner,
        this.progress.corner_material,
        TextureType.TEXTURE,
        (texture: OdysseyTexture) => {
          if (!texture) console.log('initTextures', this.progress.corner, texture);
    ...
    if (this.progress.fill.texture != '') {
      ...
      TextureLoader.enQueue(
        this.progress.fill.texture,
        this.progress.fill.material,
        TextureType.TEXTURE,
        (texture: OdysseyTexture) => {
          if (texture == null) {
  ...
  setProgress(val: number = 100) {
    ...
    if (extent.width > extent.height) {
      ...
      if (this.startFromLeft) sprite.position.x = -offsetX;
      else sprite.position.x = +offsetX;
    } else {
      sprite.scale.set(extent.width, extent.height * value, 1.0);
      const offsetY = (extent.height - extent.height * value) / 2;
      if (this.startFromLeft) sprite.position.y = +offsetY;
      else sprite.position.y = -offsetY;
    }
