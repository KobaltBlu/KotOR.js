  ...
  constructor(menu: GameMenu, control: GFFStruct, parent: GUIControl, scale: boolean = false) {
    ...
    if (this.control.hasField('DIR')) {
      ...
      if (this._dir?.hasField('IMAGE')) {
        TextureLoader.tpcLoader
          .fetch(this._dir.getFieldByLabel('IMAGE')?.getValue())
          .then((texture: OdysseyTexture) => {
            this.arrowTex = texture;

            //Up Arrow
            this.upArrowGeometry = new THREE.PlaneGeometry(1, 1, 1);
            this.upArrowMaterial = new THREE.MeshBasicMaterial({
            ...
            this.upArrow.userData.updateBox = () => {
              ...
              this.upArrow.userData.box = new THREE.Box2(
                new THREE.Vector2(-this.extent.width / 2, -this.extent.width / 2),
                new THREE.Vector2(this.extent.width / 2, this.extent.width / 2)
              );
            ...
            this.downArrowMaterial = new THREE.MeshBasicMaterial({
              color: new THREE.Color(0xffffff),
              map: this.arrowTex,
              side: THREE.DoubleSide,
            });
    ...
    if (this.control.hasField('THUMB')) {
      ...
      this.thumb.userData.box = new THREE.Box2(
        new THREE.Vector2(parentPos.x - this.extent.width / 2, parentPos.y - this.extent.height / 2),
        new THREE.Vector2(parentPos.x + this.extent.width / 2, parentPos.y + this.extent.height / 2)
      );
