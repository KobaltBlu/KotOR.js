...
export class GUILabel extends GUIControl {
  constructor(menu: GameMenu, control: GFFStruct, parent: GUIControl, scale: boolean = false) {
    ...
    this.setControlType(GUIControlType.Label);
    ...
    this.onKeyDown = (e: KeyboardEvent) => {
      ...
      switch (e.which) {
        case 8: //Backspace
          ...
          break;
        case 32: //Spacebar
          this.setText((this.getText() + ' ').substr(0, 16));
          break;
        default:
          if (e.which >= 48 && e.which <= 90) {
            if (e.shiftKey) {
              this.setText((this.getText() + String.fromCharCode(e.which).toLocaleUpperCase()).substr(0, 16));
            } else {
              this.setText((this.getText() + String.fromCharCode(e.which).toLocaleLowerCase()).substr(0, 16));
            }
          }
          break;
      }
  ...
  static _defaultClickHandler: (e: GUIControlEvent) => void = (e: GUIControlEvent) => {
    e.stopPropagation();
  };
}
