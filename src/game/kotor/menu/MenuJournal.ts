...
export class MenuJournal extends GameMenu {
  ...
  GetMenuTitle(): string {
    ...
    const sortModeLabel =
      this.sort == JournalSort.RECIEVED
        ? GameState.TLKManager.GetStringById(STRREF_BY_RECIEVED).Value
        : this.sort == JournalSort.NAME
          ? GameState.TLKManager.GetStringById(STRREF_BY_NAME).Value
          : this.sort == JournalSort.PRIORITY
            ? GameState.TLKManager.GetStringById(STRREF_BY_PRIORITY).Value
            : this.sort == JournalSort.PLANET
              ? GameState.TLKManager.GetStringById(STRREF_BY_PLANET).Value
              : '';
