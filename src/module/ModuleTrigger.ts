...
export class ModuleTrigger extends ModuleObject {
  ...
  update(delta = 0) {
    ...
    try {
      if (!this.room.model.visible) return;
    } catch (e) {}
  ...
  autoUpdateObjectsInside() {
    ...
    if (this.#autoObjectsInsideTimer < OBJECTS_INSIDE_UPDATE_THRESHOLD) {
      return;
    }
  ...
  initProperties() {
    ...
    if (this.template.RootNode.hasField('Cursor')) this.cursor = this.template.getNumberByLabel('Cursor');
    ...
    if (this.template.RootNode.hasField('KeyName')) this.keyName = this.template.getStringByLabel('KeyName');

    if (this.template.RootNode.hasField('LinkedTo')) this.linkedTo = this.template.getNumberByLabel('LinkedTo');
    ...
    if (this.template.RootNode.hasField('CreatorId')) this.creatorId = this.template.getNumberByLabel('CreatorId');

    if (this.template.RootNode.hasField('TrapFlag')) this.trapFlag = this.template.getNumberByLabel('TrapFlag');

    if (this.template.RootNode.hasField('TrapType')) this.trapType = this.template.getNumberByLabel('TrapType');

    if (this.template.RootNode.hasField('Type')) this.type = this.template.getNumberByLabel('Type');
  ...
  save() {
    ...
    //Scripts
    gff.RootNode.addField(new GFFField(GFFDataType.RESREF, ModuleObjectScript.TriggerOnClick)).setValue(
      this.scripts[ModuleObjectScript.TriggerOnClick]?.name || ''
    );
    gff.RootNode.addField(new GFFField(GFFDataType.RESREF, ModuleObjectScript.TriggerOnDisarm)).setValue(
      this.scripts[ModuleObjectScript.TriggerOnDisarm]?.name || ''
    );
    gff.RootNode.addField(new GFFField(GFFDataType.RESREF, ModuleObjectScript.TriggerOnTrapTriggered)).setValue(
      this.scripts[ModuleObjectScript.TriggerOnTrapTriggered]?.name || ''
    );
    ...
    //Scripts
    gff.RootNode.addField(new GFFField(GFFDataType.RESREF, ModuleObjectScript.TriggerOnHeartbeat)).setValue(
      this.scripts[ModuleObjectScript.TriggerOnHeartbeat]?.name || ''
    );
    gff.RootNode.addField(new GFFField(GFFDataType.RESREF, ModuleObjectScript.TriggerOnEnter)).setValue(
      this.scripts[ModuleObjectScript.TriggerOnEnter]?.name || ''
    );
    gff.RootNode.addField(new GFFField(GFFDataType.RESREF, ModuleObjectScript.TriggerOnExit)).setValue(
      this.scripts[ModuleObjectScript.TriggerOnExit]?.name || ''
    );
    gff.RootNode.addField(new GFFField(GFFDataType.RESREF, ModuleObjectScript.TriggerOnUserDefined)).setValue(
      this.scripts[ModuleObjectScript.TriggerOnUserDefined]?.name || ''
    );
    ...
    gff.RootNode.addField(new GFFField(GFFDataType.FLOAT, 'XOrientation')).setValue(
      this.template.RootNode.getNumberByLabel('XOrientation')
    );
    gff.RootNode.addField(new GFFField(GFFDataType.FLOAT, 'XPosition')).setValue(this.position.x);
    gff.RootNode.addField(new GFFField(GFFDataType.FLOAT, 'YOrientation')).setValue(
      this.template.RootNode.getNumberByLabel('YOrientation')
    );
    gff.RootNode.addField(new GFFField(GFFDataType.FLOAT, 'YPosition')).setValue(this.position.y);
    gff.RootNode.addField(new GFFField(GFFDataType.FLOAT, 'ZOrientation')).setValue(
      this.template.RootNode.getNumberByLabel('ZOrientation')
    );
