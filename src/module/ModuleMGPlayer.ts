...
export class ModuleMGPlayer extends ModuleObject {
  ...
  update(delta: number = 0) {
    ...
    if (this.camera instanceof OdysseyModel3D && this.camera.bonesInitialized && this.camera.visible) {
      ...
      if (camerahook) this.camera = camerahook.parent.parent as OdysseyModel3D;
    }
  }
  ...
  rotate(axis = 'x', amount = 0) {
    switch (axis) {
      case 'x':
        ...
        if (this.rotation.x > this.tunnel.pos.x) this.rotation.x = this.tunnel.pos.x;

        if (this.rotation.x < this.tunnel.neg.x) this.rotation.x = this.tunnel.neg.x;

        break;
      case 'y':
        this.rotation.y += amount;

        if (this.rotation.y > this.tunnel.pos.y) this.rotation.y = this.tunnel.pos.y;

        if (this.rotation.y < this.tunnel.neg.y) this.rotation.y = this.tunnel.neg.y;

        break;
      case 'z':
        this.rotation.z += amount;

        if (this.rotation.z > this.tunnel.pos.z) this.rotation.z = this.tunnel.pos.z;

        if (this.rotation.z < this.tunnel.neg.z) this.rotation.z = this.tunnel.neg.z;
  ...
  updateCollision(delta = 0) {
    if (!GameState.module || !GameState.module.area) return;
  ...
  getCurrentRoom() {
    if (this instanceof ModuleObject) {
      ...
      for (let j = 0, jl = aabbFaces.length; j < jl; j++) {
        ...
        intersects =
          castableFaces.object.collisionData.walkmesh.raycast(GameState.raycaster, castableFaces.faces) || [];
      ...
      if (room.collisionData.walkmesh) {
        for (let j = 0, jl = room.collisionData.walkmesh.walkableFaces.length; j < jl; j++) {
          ...
          if (face.triangle.containsPoint(this.position)) {
            ...
            this.position.z = this.collisionData.wm_c_point.z + 0.005;
          }
        }
      }
    }
  ...
  async loadModel() {
    for (let i = 0; i < this.modelProps.length; i++) {
      try {
        ...
        if (this.camera && model.name == this.camera.name) model.visible = false;
    ...
    if (scriptsNode) {
      for (const scriptKey of scriptResRefs) {
        if (scriptsNode.hasField(scriptKey)) {
          ...
          if (!nwscript) {
            console.warn(
              `ModuleMGPlayer.loadScripts: Failed to load script [${scriptKey}]:${resRef} for object ${this.name}`
            );
            continue;
          }
  ...
  initProperties() {
    ...
    if (this.template.RootNode.hasField('Gun_Banks')) {
      ...
      for (let i = 0; i < gun_banks.length; i++) {
        this.gunBanks.push(new ModuleMGGunBank(GFFObject.FromStruct(gun_banks[i]), this, true));
      }
