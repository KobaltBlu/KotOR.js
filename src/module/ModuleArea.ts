...
export class ModuleArea extends ModuleObject {
  ...
  dispose(){
    ...
    /*while (GameState.group.party.children.length > 1){
      GameState.group.party.children[1].dispose();
      GameState.group.party.remove(GameState.group.party.children[1]);
    }*/

    /*while (PartyManager.party.length){
      GameState.group.party.children[0].dispose();
      GameState.group.party.remove(GameState.group.party.children[0]);
    }*/
  ...
  getPlayerTemplate(){
    ...
    if(PartyManager.Player){
      ...
      pTPL.RootNode.AddField( new GFFField(GFFDataType.WORD, 'Appearance_Type') ).SetValue(GameKey == 'TSL' ? 134 : 177);
      pTPL.RootNode.AddField( new GFFField(GFFDataType.CEXOLOCSTRING, 'FirstName') ).SetValue(GameKey == 'TSL' ? 'Leia Organa' : 'Galen Urso');
      ...
      pTPL.RootNode.AddField( new GFFField(GFFDataType.WORD, 'PortraitId') ).SetValue(GameKey == 'TSL' ? 10 : 26);
      ...
      let rhStruct = new GFFStruct(UTCObject.SLOT.RIGHTHAND);
        ...
        GameState.player.Load( ( object ) => {
          ...
          GameState.player.LoadScripts( () => {
            GameState.player.LoadModel( (model) => {
              ...
              GameState.player.position.x = spawnLoc.position.x;
              GameState.player.position.y = spawnLoc.position.y;
              GameState.player.position.z = spawnLoc.position.z;
        ...
        onLoop: (room, asyncLoop) => {
          room.load( (room) => {
            if(room.model instanceof OdysseyModel3D){
              ...
              GameState.group.rooms.add(room.model);
          ...
          door.Load( ( object ) => {
            ...
            door.LoadModel( (model) => {
              door.LoadWalkmesh(model.name, (dwk) => {
                ...
                door.getCurrentRoom();
                GameState.group.doors.add( model );
