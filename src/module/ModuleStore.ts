...
export class ModuleStore extends ModuleObject {
  ...
  getMarkDown() {
    return this.markDown * 0.01;
  }

  getMarkUp() {
    return this.markUp * 0.01;
  }
  ...
  initProperties() {
    ...
    if (this.template.RootNode.hasField('MarkUp')) this.markUp = this.template.getFieldByLabel('MarkUp').getValue();
  ...
  save() {
    ...
    gff.RootNode.addField(new GFFField(GFFDataType.RESREF, 'OnOpenStore')).setValue(
      this.onOpenStore instanceof NWScriptInstance ? this.onOpenStore.name : ''
    );
