...
export class EncounterSpawnEntry {
  ...
  static FromStruct(struct: GFFStruct) {
    if (struct instanceof GFFStruct) {
      ...
      if (struct.hasField('SpawnResRef')) entry.spawnResref = struct.getFieldByLabel('SpawnResRef').getValue();

      if (struct.hasField('SpawnCR')) entry.spawnCR = struct.getFieldByLabel('SpawnCR').getValue();
