...
export class EncounterCreatureEntry {
  ...
  static FromStruct(struct: GFFStruct) {
    if (struct instanceof GFFStruct) {
      ...
      if (struct.hasField('Appearance')) entry.appearance = struct.getFieldByLabel('Appearance').getValue();

      if (struct.hasField('ResRef')) entry.resref = struct.getFieldByLabel('ResRef').getValue();

      if (struct.hasField('CR')) entry.cr = struct.getFieldByLabel('CR').getValue();

      if (struct.hasField('SingleSpawn')) entry.singleSpawn = struct.getFieldByLabel('SingleSpawn').getValue();
