...
export class ModuleCamera {
   ...
   */
  initProperties() {
    if (this.template.RootNode.hasField('CameraID')) this.cameraID = this.template.getNumberByLabel('CameraID');

    if (this.template.RootNode.hasField('FieldOfView')) this.fov = this.template.getNumberByLabel('FieldOfView');

    if (this.template.RootNode.hasField('Height')) this.height = this.template.getNumberByLabel('Height');

    if (this.template.RootNode.hasField('MicRange')) this.micRange = this.template.getNumberByLabel('MicRange');
    ...
    if (this.template.RootNode.hasField('Pitch')) this.pitch = this.template.getNumberByLabel('Pitch');
  ...
  save() {
    ...
    gff.RootNode.addField(new GFFField(GFFDataType.ORIENTATION, 'Orientation')).setValue({
    ...
    gff.RootNode.addField(new GFFField(GFFDataType.VECTOR, 'Position')).setValue({
      x: this.position.x,
      y: this.position.y,
      z: this.position.z,
    });
