...
export class EncounterSpawnPointEntry {
  ...
  static FromStruct(struct: GFFStruct) {
    if (struct instanceof GFFStruct) {
      ...
      if (struct.hasField('X')) entry.position.x = struct.getFieldByLabel('X').getValue();

      if (struct.hasField('Y')) entry.position.y = struct.getFieldByLabel('Y').getValue();

      if (struct.hasField('Z')) entry.position.z = struct.getFieldByLabel('Z').getValue();

      if (struct.hasField('Orientation')) entry.orientation = struct.getFieldByLabel('Orientation').getValue();
