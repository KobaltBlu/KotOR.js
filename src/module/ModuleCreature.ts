...
export class ModuleCreature extends ModuleObject {
    ...
    if (
      ...
      if (!this.isDead()) {
        ...
        if (
          ...
          GameState.Mode == EngineMode.DIALOG &&
          (!this.action || this.action.type != ActionType.ActionPlayAnimation)
        ) {
          if (this.model) {
            if (!this.speed) {
      ...
      if (
        !this.forceVector.length() &&
        (this.animationState.index == ModuleCreatureAnimState.RUNNING ||
          this.animationState.index == ModuleCreatureAnimState.WALKING)
      ) {
        this.setAnimationState(ModuleCreatureAnimState.IDLE);
        this.speed = 0;
        this.force = 0;
      ...
      if (this.forceVector.length()) this.collisionData.updateCollision(delta);
  ...
  updateCombat(delta = 0) {
    ...
    if (
      this.combatData.lastAttemptedAttackTarget instanceof ModuleObject &&
      this.combatData.lastAttemptedAttackTarget.isDead()
    )
      this.combatData.lastAttemptedAttackTarget = undefined;

    if (
      this.combatData.lastAttemptedSpellTarget instanceof ModuleObject &&
      this.combatData.lastAttemptedSpellTarget.isDead()
    )
      this.combatData.lastAttemptedSpellTarget = undefined;
    ...
    if (action) {
      if (action.type == ActionType.ActionCastSpell) {
        ...
        if (target instanceof ModuleObject) {
          ...
          return (
            distance <
            (this.combatData.getEquippedWeaponType() == 1 || this.combatData.getEquippedWeaponType() == 3 ? 2.0 : 15.0)
          );
        } else {
          return true;
        }
  ...
  updateAnimationState() {
    ...
    if (this.overlayAnimationState.animationName && !this.isDead()) {
      ...
      if (currentAnimation != this.overlayAnimationState.animationName) {
        if (!this.overlayAnimationState.started) {
          ...
          this.model.playOverlayAnimation(
            this.overlayAnimationState.animationName,
            this.overlayAnimationState.animation
          );
        } else {
          this.resetOverlayAnimationState();
        }
    ...
    if (GameState.Mode == EngineMode.DIALOG && this.dialogAnimation && !this.speed && !this.isDead()) return;
    ...
    if (this.animationState.animation) {
      if (currentAnimation != this.animationState.animation.name?.toLowerCase()) {
        if (!this.animationState.started) {
          ...
          let aLooping =
            !parseInt(this.animationState.animation.fireforget) && parseInt(this.animationState.animation.looping) == 1;
  ...
  damage(amount = 0, oAttacker: ModuleObject, delayTime = 0) {
    ...
    if (
      this.combatData.lastAttackTarget == undefined ||
      (this.combatData.lastAttackTarget instanceof ModuleObject && this.combatData.lastAttackTarget.isDead())
    )
      this.combatData.lastAttackTarget = oAttacker;

    if (typeof oAttacker != 'undefined') this.onDamaged();
  }
  ...
  moveToLocation(target: ModuleObject | EngineLocation, bRun = true) {
    if (target instanceof EngineLocation || target instanceof ModuleObject) {
      ...
      action.setParameter(
        4,
        ActionParameterType.DWORD,
        target instanceof EngineLocation ? ModuleObjectConstant.OBJECT_INVALID : target.id
      );
      action.setParameter(5, ActionParameterType.INT, bRun ? 1 : 0);
      action.setParameter(6, ActionParameterType.FLOAT, Math.max(1.5, distance));
      action.setParameter(7, ActionParameterType.INT, 0);
  ...
  isDueling(): boolean {
    if (!BitWise.InstanceOf(this.combatData.lastAttackTarget?.objectType, ModuleObjectType.ModuleCreature))
      return false;
  ...
  isDuelingObject(oObject: ModuleObject) {
    return (
      oObject instanceof ModuleObject &&
      this.combatData.lastAttackTarget == oObject &&
      oObject.combatData.lastAttackTarget == this &&
      oObject.combatData.getEquippedWeaponType() == 1 &&
      this.combatData.getEquippedWeaponType() == 1
    );
  }
   ...
   */
  getNearestEnemy(maxRange = 30, excludeId: number = ModuleObjectConstant.OBJECT_INVALID): ModuleCreature | undefined {
    const area = GameState.module?.area;
  ...
  attackCreature(
    ...
    if (target == this) target = GameState.PartyManager.party[0];
    ...
    switch (ev.event) {
      case 'snd_footstep':
        if (footstepSounds) {
          ...
          footstepSoundResRef = isRolling
            ? footstepSounds.getRollingResRef()
            : footstepSounds.getSurfaceSoundResRef(this.collisionData.surfaceId);
  ...
  setFacingVector(facing = new THREE.Vector3()) {
    ...
    if (this.model instanceof OdysseyModel3D)
      this.model.quaternion.setFromAxisAngle(
        new THREE.Vector3(0, 0, 1),
        -Math.atan2(Number(this.props['XOrientation']), Number(this.props['YOrientation']))
      );
  }
  ...
  setFacingObject(target: ModuleObject) {
    if (target instanceof ModuleObject) {
      this.setFacing(
        Math.atan2(this.position.y - target.position.y, this.position.x - target.position.x) + Math.PI / 2,
        false
      );
    }
  }
  ...
  onClick(callee: ModuleObject) {
    ...
    if (this.isHostile(callee) && !this.isDead()) {
      ...
      if (this.getConversation() && this.getConversation().resref) {
        GameState.getCurrentPlayer().actionDialogObject(
          this,
          this.getConversation().resref,
          false,
          undefined,
          undefined,
          true
  ...
  onCombatRoundEnd() {
    ...
    if (this.isDead() || !this.combatData.combatState) return true;
  ...
  addXP(value = 0, xpType: ExperienceType = ExperienceType.PLOT) {
    ...
    if (this.isPartyMember()) {
      GameState.UINotificationManager.EnableUINotificationIconType(
        xpType == ExperienceType.PLOT ? UIIconTimerType.PLOT_XP_RECEIVED : UIIconTimerType.STEALTH_XP_RECEIVED
      );
    }
  }
  ...
  setGoodEvil(value: number = 0) {
    ...
    if (this.isPartyMember()) {
      GameState.UINotificationManager.EnableUINotificationIconType(
        value >= oldValue ? UIIconTimerType.LIGHT_SHIFT : UIIconTimerType.DARK_SHIFT
      );
    }
  }
  ...
  addGoodEvil(value: number = 0) {
    ...
    if (this.isPartyMember()) {
      GameState.UINotificationManager.EnableUINotificationIconType(
        value >= 0 ? UIIconTimerType.LIGHT_SHIFT : UIIconTimerType.DARK_SHIFT
      );
    }
  }
  ...
  adjustAlignment(value: number = 0, alignment: number = 0) {
    ...
    if (this.isPartyMember() && (alignmentShifted == 2 || alignmentShifted == 3)) {
      GameState.UINotificationManager.EnableUINotificationIconType(
        alignmentShifted == 2 ? UIIconTimerType.LIGHT_SHIFT : UIIconTimerType.DARK_SHIFT
      );
    }
  }
  ...
  addHP(nAmount = 0, ignoreMaxHitPoints = false) {
    ...
    if (this.min1HP && this.getHP() < 1) this.setHP(1);
  }
  ...
  getMainClass() {
    if (!this.classes.length) return false;
  ...
  getRandomTalent(category = 0, category2 = 0) {
    let talents = this.getTalents().filter(
      (talent: TalentObject) => talent.category == category || talent.category == category2
    );
  ...
  getTalentBest(nCategory = 0, nCRMax = 0, nInclusion = 0, nExcludeType = -1, nExcludeId = -1) {
    let talents = this.getTalents().filter(
      (talent: TalentObject) =>
        talent.category > -1 && (talent.category & nCategory) == nCategory && talent.maxCR <= nCRMax
    );
  ...
  loadScripts() {
    ...
    for (const scriptKey of scriptKeys) {
      if (scriptsNode.hasField(scriptKey)) {
        ...
        if (!nwscript) {
          console.warn(
            `ModuleCreature.loadScripts: Failed to load script [${scriptKey}]:${resRef} for object ${this.name}`
          );
          continue;
        }
  ...
  async loadBody() {
    ...
    try {
      ...
      (model as THREE.Object3D).addEventListener(
        'playEvent' as keyof THREE.Object3DEventMap,
        this.playEvent.bind(this) as (e: THREE.Event) => void
      );
  ...
  async equipItem(slot = 0x1, item: ModuleItem) {
    ...
    switch (slot) {
      ...
      case ModuleCreatureArmorSlot.RIGHTHAND:
        ...
        await item.loadModel();
        if (item.model instanceof OdysseyModel3D) this.model.rhand.add(item.model);
        break;
      case ModuleCreatureArmorSlot.LEFTHAND:
        this.equipment.LEFTHAND = item;
        await item.loadModel();
        if (item.model instanceof OdysseyModel3D) this.model.lhand.add(item.model);
        break;
    ...
    try {
      ...
      if (this.template.RootNode.hasField('ClassList')) {
        ...
        for (let i = 0; i < classes.length; i++) {
          this.classes.push(CreatureClass.FromCreatureClassStruct(classes[i]));
        }
      }
      ...
      if (this.template.RootNode.hasField('FeatList')) {
        ...
        for (let i = 0; i < feats.length; i++) {
          this.feats.push(new TalentFeat(feats[i].getFieldByLabel('Feat').getValue()));
        }
      }
      ...
      if (this.template.RootNode.hasField('IsPC')) this.isPC = this.template.getFieldByLabel('IsPC').getValue();
      ...
      if (this.template.RootNode.hasField('Str')) this.str = this.template.getFieldByLabel('Str').getValue();

      if (this.template.RootNode.hasField('Dex')) this.dex = this.template.getFieldByLabel('Dex').getValue();

      if (this.template.RootNode.hasField('Con')) this.con = this.template.getFieldByLabel('Con').getValue();

      if (this.template.RootNode.hasField('Cha')) this.cha = this.template.getFieldByLabel('Cha').getValue();

      if (this.template.RootNode.hasField('Wis')) this.wis = this.template.getFieldByLabel('Wis').getValue();

      if (this.template.RootNode.hasField('Int')) this.int = this.template.getFieldByLabel('Int').getValue();
      ...
      if (this.template.RootNode.hasField('SWVarTable')) {
        ...
        let localNumbers = this.template.RootNode.getFieldByLabel('SWVarTable')
          .getChildStructs()[0]
          .getFieldByLabel('ByteArray')
          .getChildStructs();
        ...
        if (this.template.RootNode.hasField('Equip_ItemList')) {
          ...
          for (let i = 0; i < equipment.length; i++) {
            ...
            if (strt.hasField('EquippedRes')) {
              equipped_item = new GameState.Module.ModuleArea.ModuleItem(
                strt.getFieldByLabel('EquippedRes').getValue()
              );
            } else {
  ...
  playSoundSet(type = -1) {
    ...
    if (resref != '') {
      if (this.audioEmitter) this.audioEmitter.playSoundFireAndForget(resref);
    }
  }
  ...
  save() {
    ...
    combatInfoContent
      .addField(new GFFField(GFFDataType.BYTE, 'NumAttacks'))
      .setValue(this.combatRound?.onHandAttacks ?? 1);
    combatInfoContent
      .addField(new GFFField(GFFDataType.CHAR, 'OnHandAttackMod'))
      .setValue(bab + (rHand?.getAttackBonus?.() ?? 0));
    combatInfoContent.addField(new GFFField(GFFDataType.CHAR, 'OnHandDamageMod')).setValue(0);
    combatInfoContent
      .addField(new GFFField(GFFDataType.CHAR, 'OffHandAttackMod'))
      .setValue(lHand ? Math.floor(bab / 2) + (lHand.getAttackBonus?.() ?? 0) : 0);
    ...
    combatInfoContent
      .addField(new GFFField(GFFDataType.BYTE, 'OnHandCritRng'))
      .setValue(rHand?.getCriticalThreatRangeMin?.() ?? 20);
    combatInfoContent
      .addField(new GFFField(GFFDataType.BYTE, 'OnHandCritMult'))
      .setValue(rHand?.baseItem?.criticalHitMultiplier ?? 2);
    combatInfoContent.addField(new GFFField(GFFDataType.BYTE, 'OffHandWeaponEq')).setValue(lHand ? 1 : 0);
    combatInfoContent
      .addField(new GFFField(GFFDataType.BYTE, 'OffHandCritRng'))
      .setValue(lHand?.getCriticalThreatRangeMin?.() ?? 20);
    combatInfoContent
      .addField(new GFFField(GFFDataType.BYTE, 'OffHandCritMult'))
      .setValue(lHand?.baseItem?.criticalHitMultiplier ?? 2);
    ...
    gff.RootNode.addField(new GFFField(GFFDataType.WORD, 'FactionID')).setValue(
    ...
    gff.RootNode.addField(new GFFField(GFFDataType.CEXOLOCSTRING, 'FirstName')).setValue(
      this.template.RootNode.getFieldByLabel('FirstName')?.getCExoLocString()
    );
    ...
    gff.RootNode.addField(new GFFField(GFFDataType.CEXOLOCSTRING, 'LastName')).setValue(
      this.template.RootNode.getFieldByLabel('LastName')?.getCExoLocString()
    );
    ...
    gff.RootNode.addField(new GFFField(GFFDataType.BYTE, 'PM_IsDisguised')).setValue(
      this.hasEffect(GameEffectType.EffectDisguise) ? 1 : 0
    );
    ...
    //Scripts
    gff.RootNode.addField(new GFFField(GFFDataType.RESREF, 'ScriptAttacked')).setValue(
      this.scripts[ModuleObjectScript.CreatureOnAttacked]?.name || ''
    );
    gff.RootNode.addField(new GFFField(GFFDataType.RESREF, 'ScriptDamaged')).setValue(
      this.scripts[ModuleObjectScript.CreatureOnDamaged]?.name || ''
    );
    gff.RootNode.addField(new GFFField(GFFDataType.RESREF, 'ScriptDeath')).setValue(
      this.scripts[ModuleObjectScript.CreatureOnDeath]?.name || ''
    );
    gff.RootNode.addField(new GFFField(GFFDataType.RESREF, 'ScriptDialogue')).setValue(
      this.scripts[ModuleObjectScript.CreatureOnDialog]?.name || ''
    );
    gff.RootNode.addField(new GFFField(GFFDataType.RESREF, 'ScriptDisturbed')).setValue(
      this.scripts[ModuleObjectScript.CreatureOnDisturbed]?.name || ''
    );
    gff.RootNode.addField(new GFFField(GFFDataType.RESREF, 'ScriptEndDialogu')).setValue(
      this.scripts[ModuleObjectScript.CreatureOnEndDialog]?.name || ''
    );
    gff.RootNode.addField(new GFFField(GFFDataType.RESREF, 'ScriptEndRound')).setValue(
      this.scripts[ModuleObjectScript.CreatureOnEndRound]?.name || ''
    );
    gff.RootNode.addField(new GFFField(GFFDataType.RESREF, 'ScriptHeartbeat')).setValue(
      this.scripts[ModuleObjectScript.CreatureOnHeartbeat]?.name || ''
    );
    gff.RootNode.addField(new GFFField(GFFDataType.RESREF, 'ScriptOnBlocked')).setValue(
      this.scripts[ModuleObjectScript.CreatureOnBlocked]?.name || ''
    );
    gff.RootNode.addField(new GFFField(GFFDataType.RESREF, 'ScriptOnNotice')).setValue(
      this.scripts[ModuleObjectScript.CreatureOnNotice]?.name || ''
    );
    gff.RootNode.addField(new GFFField(GFFDataType.RESREF, 'ScriptRested')).setValue(
      this.scripts[ModuleObjectScript.CreatureOnRested]?.name || ''
    );
    gff.RootNode.addField(new GFFField(GFFDataType.RESREF, 'ScriptSpawn')).setValue(
      this.scripts[ModuleObjectScript.CreatureOnSpawn]?.name || ''
    );
    gff.RootNode.addField(new GFFField(GFFDataType.RESREF, 'ScriptSpellAt')).setValue(
      this.scripts[ModuleObjectScript.CreatureOnSpellAt]?.name || ''
    );
    gff.RootNode.addField(new GFFField(GFFDataType.RESREF, 'ScriptUserDefine')).setValue(
      this.scripts[ModuleObjectScript.CreatureOnUserDefined]?.name || ''
    );
    ...
    gff.RootNode.addField(new GFFField(GFFDataType.WORD, 'SkillPoints')).setValue(
      this.skillPoints ? this.skillPoints : 0
    );
