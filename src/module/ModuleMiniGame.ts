...
export class ModuleMiniGame {
  ...
  constructor(struct: GFFStruct) {
    ...
    if (struct.hasField('Music'))
      this.music =
        typeof struct.getFieldByLabel('Music').getValue() === 'string'
          ? struct.getFieldByLabel('Music').getValue()
          : '';
    ...
    if (struct.hasField('Player')) {
      ...
      if (playerStructs.length > 0) {
        this.player = new GameState.Module.ModuleArea.ModuleMGPlayer(GFFObject.FromStruct(playerStructs[0]));
      }
    }
    if (struct.hasField('Enemies')) {
      const enemyList = struct.getFieldByLabel('Enemies').getChildStructs();
      for (let i = 0; i < enemyList.length; i++) {
        this.enemies.push(new ModuleMGEnemy(GFFObject.FromStruct(enemyList[i])));
      }
    }
  }
