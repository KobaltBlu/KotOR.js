...
export class ModuleAreaOfEffect extends ModuleObject {
  ...
  initProperties(): void {
    ...
    if (this.template.RootNode.hasField('Shape')) this.shape = this.template.getFieldByLabel('Shape').getValue();

    //RECT
    if (this.shape == AreaOfEffectShape.RECTANGLE) {
      if (this.template.RootNode.hasField('Length')) this.length = this.template.getFieldByLabel('Length').getValue();

      if (this.template.RootNode.hasField('Width')) this.width = this.template.getFieldByLabel('Width').getValue();
    }
    //CIRCLE
    else {
      if (this.template.RootNode.hasField('Radius')) this.radius = this.template.getFieldByLabel('Radius').getValue();
    }

    if (this.template.RootNode.hasField('SpellId')) this.spellId = this.template.getFieldByLabel('SpellId').getValue();
    ...
    if (this.template.RootNode.hasField('SWVarTable')) {
      ...
      const localNumbers = this.template.RootNode.getFieldByLabel('SWVarTable')
        .getChildStructs()[0]
  ...
  save(): GFFObject {
    ...
    gff.RootNode.addField(new GFFField(GFFDataType.DWORD, 'LinkedToObject')).setValue(
      this.linkedToObject instanceof ModuleObject ? this.linkedToObject.id : ModuleObjectConstant.OBJECT_INVALID
    );

    gff.RootNode.addField(new GFFField(GFFDataType.DWORD, 'CreatorId')).setValue(
      this.creator instanceof ModuleObject ? this.creator.id : ModuleObjectConstant.OBJECT_INVALID
    );

    gff.RootNode.addField(new GFFField(GFFDataType.DWORD, 'LastEntered')).setValue(
      this.lastEntered instanceof ModuleObject ? this.lastEntered.id : ModuleObjectConstant.OBJECT_INVALID
    );

    gff.RootNode.addField(new GFFField(GFFDataType.DWORD, 'LastLeft')).setValue(
      this.lastLeft instanceof ModuleObject ? this.lastLeft.id : ModuleObjectConstant.OBJECT_INVALID
    );
