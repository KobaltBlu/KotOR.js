...
export class PazaakDeck {
  ...
  static ModifierToCardIndex(cardModifier: string) {
    if (cardModifier == '+1') {
      ...
      return 17;
    } else if (cardModifier == '$$') {
      //TSL only
      return 18;
    } else if (cardModifier == 'F1') {
      //TSL only
      return 19;
    } else if (cardModifier == 'F2') {
      //TSL only
      return 20;
    } else if (cardModifier == 'TT') {
      //TSL only
      return 21;
    } else if (cardModifier == 'VV') {
      //TSL only
      return 22;
    }
    return -1;
  ...
  static From2DA(row: any = {}) {
    ...
    if (row.hasOwnProperty('card0')) deck.card0 = TwoDAObject.normalizeValue(row.card0, 'string', '') as string;

    if (row.hasOwnProperty('card1')) deck.card1 = TwoDAObject.normalizeValue(row.card1, 'string', '') as string;

    if (row.hasOwnProperty('card2')) deck.card2 = TwoDAObject.normalizeValue(row.card2, 'string', '') as string;

    if (row.hasOwnProperty('card3')) deck.card3 = TwoDAObject.normalizeValue(row.card3, 'string', '') as string;

    if (row.hasOwnProperty('card4')) deck.card4 = TwoDAObject.normalizeValue(row.card4, 'string', '') as string;

    if (row.hasOwnProperty('card5')) deck.card5 = TwoDAObject.normalizeValue(row.card5, 'string', '') as string;

    if (row.hasOwnProperty('card6')) deck.card6 = TwoDAObject.normalizeValue(row.card6, 'string', '') as string;

    if (row.hasOwnProperty('card7')) deck.card7 = TwoDAObject.normalizeValue(row.card7, 'string', '') as string;

    if (row.hasOwnProperty('card8')) deck.card8 = TwoDAObject.normalizeValue(row.card8, 'string', '') as string;

    if (row.hasOwnProperty('card9')) deck.card9 = TwoDAObject.normalizeValue(row.card9, 'string', '') as string;
