...
export class Reputation {
  ...
  static FromStruct(struct: GFFStruct) {
    if (struct instanceof GFFStruct) {
      ...
      if (struct.hasField('FactionID1')) reputation.id1 = struct.getNumberByLabel('FactionID1');

      if (struct.hasField('FactionID2')) reputation.id2 = struct.getNumberByLabel('FactionID2');

      if (struct.hasField('FactionRep')) reputation.reputation = struct.getNumberByLabel('FactionRep');
