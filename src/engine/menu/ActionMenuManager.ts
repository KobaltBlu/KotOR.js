...
export class ActionMenuManager {
  ...
  static UpdateMenuActions() {
      ...
      if (ActionMenuManager.oTarget instanceof GameState.Module.ModuleArea.ModulePlaceable) {
        if (ActionMenuManager.oTarget.isLocked()) {
          if (bHasSecuritySkill) {
            ...
            ActionMenuManager.ActionPanels.targetPanels[1].addAction(
              new GameState.ActionMenuManager.ActionMenuItem({
                action: action,
          ...
          if (!this.oTarget?.notBlastable) {
            ActionMenuManager.ActionPanels.targetPanels[0].addAction(
              new GameState.ActionMenuManager.ActionMenuItem({
                action: {
          ...
          if (!ActionMenuManager.oTarget?.notBlastable) {
            ...
            for (let i = 0, len = mineList.length; i < len; i++) {
              ...
              ActionMenuManager.ActionPanels.targetPanels[2].addAction(
                new GameState.ActionMenuManager.ActionMenuItem({
                  action: setMine,
                  icon: item.getIcon(),
                })
        ...
        ActionMenuManager.oTarget instanceof GameState.Module.ModuleArea.ModuleCreature &&
        ActionMenuManager.oTarget.isHostile(GameState.PartyManager.party[0] as ModuleObject)
      ) {
        ActionMenuManager.ActionPanels.targetPanels[0].addAction(
          new GameState.ActionMenuManager.ActionMenuItem({
            action: {
        ...
        if (ActionMenuManager.oPC.getEquippedWeaponType() == 1) {
          ...
          const feats = ActionMenuManager.oPC.getFeats().filter((f: any, i: number, array: any[]) => {
            return (
              f.category == 0x1104 && (f.successor == '****' || !array.find((f2: any) => f2.__index == f.successor))
            );
          });
        ...
        if (ActionMenuManager.oPC.getEquippedWeaponType() == 4) {
          ...
          const feats = ActionMenuManager.oPC.getFeats().filter((f: any, i: number, array: any[]) => {
            return (
              f.category == 0x1111 && (f.successor == '****' || !array.find((f2: any) => f2.__index == f.successor))
            );
          });
    ...
    const friendlySpells = ActionMenuManager.oPC.getSpells().filter((s: any) => {
      const friendly = parseInt(s.forcefriendly);
      const hostile = parseInt(s.forcehostile);
      return friendly === 1 && (isNaN(hostile) || hostile === 0);
    });

    for (let i = 0; i < friendlySpells.length; i++) {
      ActionMenuManager.ActionPanels.selfPanels[1].addAction(
        new GameState.ActionMenuManager.ActionMenuItem({
  ...
  static targetActionCount() {
    return ActionMenuManager.ActionPanels.targetPanels.reduce((previousValue, currentValue, currentIndex, panels) => {
      return (previousValue += currentValue.actions.length);
    }, 0);
  }

  static selfActionCount() {
    return ActionMenuManager.ActionPanels.selfPanels.reduce((previousValue, currentValue, currentIndex, panels) => {
      return (previousValue += currentValue.actions.length);
    }, 0);
  }

  static onTargetMenuAction(index: number = 0) {
    if (!ActionMenuManager.oPC) {
      return;
    }
    ...
    if (action) {
      if (index == 0) {
        if (action.action && action.action.type == ActionType.ActionPhysicalAttacks) {
          ...
          ActionMenuManager.oPC.actionQueue.addFront(action.action);
