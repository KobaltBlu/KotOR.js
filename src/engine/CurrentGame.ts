...
export class CurrentGame {
  ...
  static IsModuleSaved(name = '') {
    return new Promise((resolve, reject) => {
      GameFileSystem.readdir(CurrentGame.gameinprogress_dir)
        .then((files) => {
      ...
      if (ApplicationProfile.ENV == ApplicationEnvironment.ELECTRON) {
        ...
        try {
          ...
          if (directory_handle instanceof FileSystemDirectoryHandle) {
            ...
            const directory_handle = await ApplicationProfile.directoryHandle.getDirectoryHandle(
              CurrentGame.gameinprogress_dir,
              { create: true }
            );
