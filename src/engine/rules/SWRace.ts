...
export class SWRace {
  ...
  static From2DA(row: Record<string, unknown> = {}): SWRace {
    ...
    if (row.hasOwnProperty('label')) race.label = TwoDAObject.normalizeValue(row.label, 'string', '') as string;

    if (row.hasOwnProperty('abrev')) race.abrev = TwoDAObject.normalizeValue(row.abrev, 'string', '') as string;

    if (row.hasOwnProperty('name')) race.name = TwoDAObject.normalizeValue(row.name, 'number', -1) as number;
    ...
    if (row.hasOwnProperty('favored')) race.favored = TwoDAObject.normalizeValue(row.favored, 'number', -1) as number;
    ...
    if (row.hasOwnProperty('age')) race.age = TwoDAObject.normalizeValue(row.age, 'number', 18) as number;
