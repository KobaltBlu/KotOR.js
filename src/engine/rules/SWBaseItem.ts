...
export class SWBaseItem {
  ...
  static From2DA(row: any = {}): SWBaseItem {
    ...
    if (row.hasOwnProperty('__index')) baseItem.id = TwoDAObject.normalizeValue(row.__index, 'number', 0) as number;
    if (row.hasOwnProperty('name')) baseItem.name = TwoDAObject.normalizeValue(row.name, 'number', 0) as number;
    if (row.hasOwnProperty('label')) baseItem.label = TwoDAObject.normalizeValue(row.label, 'string', '') as string;
    ...
    if (row.hasOwnProperty('baseac')) baseItem.baseAC = TwoDAObject.normalizeValue(row.baseac, 'number', 0) as number;
