...
export class SWItemPropsDef {
  ...
  static From2DA(
    ...
    if (Object.hasOwn(row, 'name')) propsDef.name = TwoDAObject.normalizeValue(row.name, 'number', -1) as number;

    if (Object.hasOwn(row, 'label')) propsDef.label = TwoDAObject.normalizeValue(row.label, 'string', '') as string;

    if (Object.hasOwn(row, 'subtyperesref'))
      propsDef.subtyperesref = TwoDAObject.normalizeValue(row.subtyperesref, 'string', '') as string;

    if (Object.hasOwn(row, 'cost')) propsDef.cost = TwoDAObject.normalizeValue(row.cost, 'number', -1) as number;
