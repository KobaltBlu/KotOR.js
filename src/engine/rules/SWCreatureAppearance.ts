...
export class SWCreatureAppearance {
  ...
  static From2DA(row: any = {}): SWCreatureAppearance {
    ...
    if (row.hasOwnProperty('label')) appearance.label = TwoDAObject.normalizeValue(row.label, 'string', '');
    if (row.hasOwnProperty('string_ref'))
      appearance.string_ref = TwoDAObject.normalizeValue(row.string_ref, 'number', -1);
    if (row.hasOwnProperty('race')) appearance.race = TwoDAObject.normalizeValue(row.race, 'string', '');
    if (row.hasOwnProperty('walkdist')) appearance.walkdist = TwoDAObject.normalizeValue(row.walkdist, 'number', 1.7);
    if (row.hasOwnProperty('rundist')) appearance.rundist = TwoDAObject.normalizeValue(row.rundist, 'number', 5.4);
    ...
    if (row.hasOwnProperty('racetex')) appearance.racetex = TwoDAObject.normalizeValue(row.racetex, 'string', '');
    ...
    if (row.hasOwnProperty('modela')) appearance.modela = TwoDAObject.normalizeValue(row.modela, 'string', '');
    if (row.hasOwnProperty('texa')) appearance.texa = TwoDAObject.normalizeValue(row.texa, 'string', '');
    if (row.hasOwnProperty('texaevil')) appearance.texaevil = TwoDAObject.normalizeValue(row.texaevil, 'string', '');

    if (row.hasOwnProperty('modelb')) appearance.modelb = TwoDAObject.normalizeValue(row.modelb, 'string', '');
    if (row.hasOwnProperty('texb')) appearance.texb = TwoDAObject.normalizeValue(row.texb, 'string', '');
    if (row.hasOwnProperty('texbevil')) appearance.texbevil = TwoDAObject.normalizeValue(row.texbevil, 'string', ''); //TSL

    if (row.hasOwnProperty('modelc')) appearance.modelc = TwoDAObject.normalizeValue(row.modelc, 'string', '');
    if (row.hasOwnProperty('texc')) appearance.texc = TwoDAObject.normalizeValue(row.texc, 'string', '');

    if (row.hasOwnProperty('modeld')) appearance.modeld = TwoDAObject.normalizeValue(row.modeld, 'string', '');
    if (row.hasOwnProperty('texd')) appearance.texd = TwoDAObject.normalizeValue(row.texd, 'string', '');

    if (row.hasOwnProperty('modele')) appearance.modele = TwoDAObject.normalizeValue(row.modele, 'string', '');
    if (row.hasOwnProperty('texe')) appearance.texe = TwoDAObject.normalizeValue(row.texe, 'string', '');

    if (row.hasOwnProperty('modelf')) appearance.modelf = TwoDAObject.normalizeValue(row.modelf, 'string', '');
    if (row.hasOwnProperty('texf')) appearance.texf = TwoDAObject.normalizeValue(row.texf, 'string', '');

    if (row.hasOwnProperty('modelg')) appearance.modelg = TwoDAObject.normalizeValue(row.modelg, 'string', '');
    if (row.hasOwnProperty('texg')) appearance.texg = TwoDAObject.normalizeValue(row.texg, 'string', '');

    if (row.hasOwnProperty('modelh')) appearance.modelh = TwoDAObject.normalizeValue(row.modelh, 'string', '');
    if (row.hasOwnProperty('texh')) appearance.texh = TwoDAObject.normalizeValue(row.texh, 'string', '');

    if (row.hasOwnProperty('modeli')) appearance.modeli = TwoDAObject.normalizeValue(row.modeli, 'string', '');
    if (row.hasOwnProperty('texi')) appearance.texi = TwoDAObject.normalizeValue(row.texi, 'string', '');
    if (row.hasOwnProperty('texievil')) appearance.texievil = TwoDAObject.normalizeValue(row.texievil, 'string', ''); //TSL

    if (row.hasOwnProperty('modelj')) appearance.modelj = TwoDAObject.normalizeValue(row.modelj, 'string', '');
    if (row.hasOwnProperty('texj')) appearance.texj = TwoDAObject.normalizeValue(row.texj, 'string', '');

    if (row.hasOwnProperty('modelk')) appearance.modelk = TwoDAObject.normalizeValue(row.modelk, 'string', ''); //TSL
    if (row.hasOwnProperty('texk')) appearance.texk = TwoDAObject.normalizeValue(row.texk, 'string', ''); //TSL

    if (row.hasOwnProperty('modell')) appearance.modell = TwoDAObject.normalizeValue(row.modell, 'string', ''); //TSL
    if (row.hasOwnProperty('texl')) appearance.texl = TwoDAObject.normalizeValue(row.texl, 'string', ''); //TSL

    if (row.hasOwnProperty('modelm')) appearance.modelm = TwoDAObject.normalizeValue(row.modelm, 'string', ''); //TSL
    if (row.hasOwnProperty('texm')) appearance.texm = TwoDAObject.normalizeValue(row.texm, 'string', ''); //TSL

    if (row.hasOwnProperty('modeln')) appearance.modeln = TwoDAObject.normalizeValue(row.modeln, 'string', ''); //TSL
    if (row.hasOwnProperty('texn')) appearance.texn = TwoDAObject.normalizeValue(row.texn, 'string', ''); //TSL
    if (row.hasOwnProperty('texnevil')) appearance.texnevil = TwoDAObject.normalizeValue(row.texnevil, 'string', ''); //TSL

    if (row.hasOwnProperty('skin')) appearance.skin = TwoDAObject.normalizeValue(row.skin, 'string', '');
    if (row.hasOwnProperty('headtexve')) appearance.headtexve = TwoDAObject.normalizeValue(row.headtexve, 'string', '');
    if (row.hasOwnProperty('headtexe')) appearance.headtexe = TwoDAObject.normalizeValue(row.headtexe, 'string', '');
    if (row.hasOwnProperty('headtexg')) appearance.headtexg = TwoDAObject.normalizeValue(row.headtexg, 'string', '');
    if (row.hasOwnProperty('headtexvg')) appearance.headtexvg = TwoDAObject.normalizeValue(row.headtexvg, 'string', '');
    if (row.hasOwnProperty('envmap')) appearance.envmap = TwoDAObject.normalizeValue(row.envmap, 'string', '');
    ...
    if (row.hasOwnProperty('driveaccl')) appearance.driveaccl = TwoDAObject.normalizeValue(row.driveaccl, 'number', 50);
    ...
    if (row.hasOwnProperty('perspace')) appearance.perspace = TwoDAObject.normalizeValue(row.perspace, 'number', 0.35);
    ...
    if (row.hasOwnProperty('height')) appearance.height = TwoDAObject.normalizeValue(row.height, 'number', 0);
    ...
    if (row.hasOwnProperty('haslegs')) appearance.haslegs = TwoDAObject.normalizeValue(row.haslegs, 'boolean', true);
    if (row.hasOwnProperty('hasarms')) appearance.hasarms = TwoDAObject.normalizeValue(row.hasarms, 'boolean', true);
    ...
    if (row.hasOwnProperty('headtrack')) appearance.headtrack = TwoDAObject.normalizeValue(row.headtrack, 'number', 0);
    ...
    if (row.hasOwnProperty('headbone')) appearance.headbone = TwoDAObject.normalizeValue(row.headbone, 'string', '');
    if (row.hasOwnProperty('hitdist')) appearance.hitdist = TwoDAObject.normalizeValue(row.hitdist, 'number', 1);
    ...
    if (row.hasOwnProperty('body_bag')) appearance.body_bag = TwoDAObject.normalizeValue(row.body_bag, 'number', -1);
    ...
    if (row.hasOwnProperty('deathfx')) appearance.deathfx = TwoDAObject.normalizeValue(row.deathfx, 'number', -1);
