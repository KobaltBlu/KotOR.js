...
export class SWPriorityGroup {
  ...
  static From2DA(row: Record<string, unknown> = {}): SWPriorityGroup {
    ...
    if (row.hasOwnProperty('label')) group.label = TwoDAObject.normalizeValue(row.label, 'string', '') as string;

    if (row.hasOwnProperty('priority'))
      group.priority = TwoDAObject.normalizeValue(row.priority, 'number', 0) as number;

    if (row.hasOwnProperty('volume')) group.volume = TwoDAObject.normalizeValue(row.volume, 'number', 0) as number;
