...
export class SWPortrait {
  ...
  static From2DA(row: any = {}) {
    ...
    if (row.hasOwnProperty('__index')) portrait.id = TwoDAObject.normalizeValue(row.__index, 'number', 0);
    ...
    if (row.hasOwnProperty('sex')) portrait.sex = TwoDAObject.normalizeValue(row.sex, 'number', 0);

    if (row.hasOwnProperty('race')) portrait.race = TwoDAObject.normalizeValue(row.race, 'number', 0);

    if (row.hasOwnProperty('inanimatetype'))
      portrait.inanimatetype = TwoDAObject.normalizeValue(row.inanimatetype, 'number', 0);

    if (row.hasOwnProperty('plot')) portrait.plot = TwoDAObject.normalizeValue(row.plot, 'number', 0);

    if (row.hasOwnProperty('lowgore')) portrait.lowgore = TwoDAObject.normalizeValue(row.lowgore, 'number', 0);
    ...
    if (row.hasOwnProperty('forpc')) portrait.forpc = TwoDAObject.normalizeValue(row.forpc, 'number', 0);
