...
export class SWIPDamageCost {
  ...
  static From2DA(row: any = {}){
    const propsDef = new SWIPDamageCost();
    ...
    if(row.hasOwnProperty('rank'))
      propsDef.rank = TwoDAObject.normalizeValue(row.rank, 'number', 0) as number;
    
    if(row.hasOwnProperty('gamestring'))
      propsDef.gamestring = TwoDAObject.normalizeValue(row.gamestring, 'number', 0) as number;
