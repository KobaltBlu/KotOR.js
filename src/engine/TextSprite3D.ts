...
export class TextSprite3D {
  ...
  buildText() {
    ...
    if (
      this.text.text != '' ||
      (this.text.strref != 0 && typeof TLKManager.TLKStrings[this.text.strref] != 'undefined')
    )
  ...
  static CreateOnObject(
    object: ModuleObject,
    text: string = '',
    type: TextSprite3DType = TextSprite3DType.NEUTRAL,
    timer: number = 3000
  ) {
    if (!object) return;
