...
export class MenuManager {
  ...
  static Add(menu: GameMenu) {
    ...
    if (!menu.isOverlayGUI) {
      ...
      if (MenuManager.activeMenus.length) MenuManager.activeMenus[MenuManager.activeMenus.length - 1].hide();

      const idx = MenuManager.activeMenus.indexOf(menu);
      if (idx >= 0) MenuManager.activeMenus.splice(idx, 1);

      MenuManager.activeMenus.push(menu);

      MenuManager.Resize();
    } else {
      if (MenuManager.activeModals.indexOf(menu) == -1) MenuManager.activeModals.push(menu);
    }
  }
  ...
  static Remove(menu: GameMenu) {
    ...
    if (!menu.isOverlayGUI) {
      ...
      if (mIdx >= 0) MenuManager.activeMenus.splice(mIdx, 1);

      //Reshow the new top most menu in the list
      if (MenuManager.activeMenus.length) MenuManager.GetCurrentMenu().show();

      MenuManager.Resize();

      if (
        (!MenuManager.activeMenus.length ||
          MenuManager.activeMenus[MenuManager.activeMenus.length - 1].engineMode != EngineMode.GUI) &&
      ...
      if (mIdx >= 0) MenuManager.activeModals.splice(mIdx, 1);
    }
  }
  ...
  static async LoadMainGameMenus() {
    try {
      if (GameState.GameKey == GameEngineType.KOTOR) {
        ...
        MenuManager.MenuResolutions = (await MenuManager.GameMenuLoader(
          KOTOR.MenuResolutions
        )) as KOTOR.MenuResolutions;
        ...
        MenuManager.MenuGraphicsAdvanced = (await MenuManager.GameMenuLoader(
          KOTOR.MenuGraphicsAdvanced
        )) as KOTOR.MenuGraphicsAdvanced;
        ...
        MenuManager.MenuSoundAdvanced = (await MenuManager.GameMenuLoader(
          KOTOR.MenuSoundAdvanced
        )) as KOTOR.MenuSoundAdvanced;
        MenuManager.MenuMouse = (await MenuManager.GameMenuLoader(KOTOR.MenuMouse)) as KOTOR.MenuMouse;
        MenuManager.MenuKeyboardMapping = (await MenuManager.GameMenuLoader(
          KOTOR.MenuKeyboardMapping
        )) as KOTOR.MenuKeyboardMapping;
        MenuManager.MenuKeyboardEntry = (await MenuManager.GameMenuLoader(
          KOTOR.MenuKeyboardEntry
        )) as KOTOR.MenuKeyboardEntry;
  ...
  static async LoadCharGenGameMenus() {
    try {
      if (GameState.GameKey == GameEngineType.KOTOR) {
        ...
        MenuManager.CharGenAbilities = (await MenuManager.GameMenuLoader(
          KOTOR.CharGenAbilities
        )) as KOTOR.CharGenAbilities;
        MenuManager.CharGenClass = (await MenuManager.GameMenuLoader(KOTOR.CharGenClass)) as KOTOR.CharGenClass;
        MenuManager.CharGenCustomPanel = (await MenuManager.GameMenuLoader(
          KOTOR.CharGenCustomPanel
        )) as KOTOR.CharGenCustomPanel;
        ...
        MenuManager.CharGenPortCust = (await MenuManager.GameMenuLoader(
          KOTOR.CharGenPortCust
        )) as KOTOR.CharGenPortCust; //Character Portrait
        MenuManager.CharGenQuickOrCustom = (await MenuManager.GameMenuLoader(
          KOTOR.CharGenQuickOrCustom
        )) as KOTOR.CharGenQuickOrCustom;
        MenuManager.CharGenQuickPanel = (await MenuManager.GameMenuLoader(
          KOTOR.CharGenQuickPanel
        )) as KOTOR.CharGenQuickPanel;
        MenuManager.CharGenSkills = (await MenuManager.GameMenuLoader(KOTOR.CharGenSkills)) as KOTOR.CharGenSkills;
      } else if (GameState.GameKey == GameEngineType.TSL) {
        MenuManager.CharGenAbilities = (await MenuManager.GameMenuLoader(
          TSL.CharGenAbilities
        )) as KOTOR.CharGenAbilities;
        MenuManager.CharGenClass = (await MenuManager.GameMenuLoader(TSL.CharGenClass)) as KOTOR.CharGenClass;
        MenuManager.CharGenCustomPanel = (await MenuManager.GameMenuLoader(
          TSL.CharGenCustomPanel
        )) as KOTOR.CharGenCustomPanel;
        ...
        MenuManager.CharGenQuickOrCustom = (await MenuManager.GameMenuLoader(
          TSL.CharGenQuickOrCustom
        )) as KOTOR.CharGenQuickOrCustom;
        MenuManager.CharGenQuickPanel = (await MenuManager.GameMenuLoader(
          TSL.CharGenQuickPanel
        )) as KOTOR.CharGenQuickPanel;
  ...
  static async LoadInGameMenus() {
    ...
    try {
      if (GameState.GameKey == GameEngineType.KOTOR) {
        MenuManager.InGameAreaTransition = (await MenuManager.GameMenuLoader(
          KOTOR.InGameAreaTransition
        )) as KOTOR.InGameAreaTransition;
        ...
        MenuManager.InGameComputerCam = (await MenuManager.GameMenuLoader(
          KOTOR.InGameComputerCam
        )) as KOTOR.InGameComputerCam;
        ...
        MenuManager.MenuPartySelection = (await MenuManager.GameMenuLoader(
          KOTOR.MenuPartySelection
        )) as KOTOR.MenuPartySelection;
        ...
        MenuManager.MenuUpgradeItems = (await MenuManager.GameMenuLoader(
          KOTOR.MenuUpgradeItems
        )) as KOTOR.MenuUpgradeItems;
        MenuManager.MenuUpgradeSelect = (await MenuManager.GameMenuLoader(
          KOTOR.MenuUpgradeSelect
        )) as KOTOR.MenuUpgradeSelect;
        MenuManager.MenuPazaakWager = (await MenuManager.GameMenuLoader(
          KOTOR.MenuPazaakWager
        )) as KOTOR.MenuPazaakWager;
        MenuManager.MenuPazaakGame = (await MenuManager.GameMenuLoader(KOTOR.MenuPazaakGame)) as KOTOR.MenuPazaakGame;
        MenuManager.MenuPazaakSetup = (await MenuManager.GameMenuLoader(
          KOTOR.MenuPazaakSetup
        )) as KOTOR.MenuPazaakSetup;
      } else if (GameState.GameKey == GameEngineType.TSL) {
        MenuManager.InGameAreaTransition = (await MenuManager.GameMenuLoader(
          TSL.InGameAreaTransition
        )) as KOTOR.InGameAreaTransition;
        ...
        MenuManager.MenuUpgradeItems = (await MenuManager.GameMenuLoader(
          TSL.MenuUpgradeItems
        )) as KOTOR.MenuUpgradeItems;
        MenuManager.MenuUpgradeSelect = (await MenuManager.GameMenuLoader(
          TSL.MenuUpgradeSelect
        )) as KOTOR.MenuUpgradeSelect;
