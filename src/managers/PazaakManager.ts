...
export class PazaakManager {
  ...
  static SetOpponentDeck(deckIndex: number) {
    const deckConfig = GameState.SWRuleSet.pazaakDecks[deckIndex] || GameState.SWRuleSet.pazaakDecks[0];
  ...
  static ProcessActionQueue(delta: number) {
    ...
    if (action.type == PazaakActionType.WAIT) {
      ...
      console.log(
        `PazaakManager: End turn ${tableIndex == 0 ? 'Player' : 'Opponent'} [${bStanding ? 'standing' : 'not standing'}]`
      );
      const table = this.Tables[tableIndex];
      ...
      console.log(
        `PazaakManager: End game ${winner == PazaakTurnMode.PLAYER ? 'Player' : 'Opponent'} wins! | ${winnings} gold`
      );
      ...
      console.log(
        `PazaakManager: Play hand card ${tableIndex == 0 ? 'Player' : 'Opponent'} ${handIndex} ${flipped ? 'flipped' : 'not flipped'}`
      );
      ...
      for (let i = 0; i < PazaakTableSlots.MAX_SLOTS; i++) {
        ...
        console.log(
          `PazaakManager: Play hand card ${tableIndex == 0 ? 'Player' : 'Opponent'} ${i + 1} ${flipped ? 'flipped' : 'not flipped'}`
        );
  ...
  static BuildAction(tableIndex: number, actionType: PazaakActionType, properties: any[] = []) {
    ...
    for (let i = 0; i < properties.length; i++) {
      const type =
        typeof properties[i] === 'string' ? PazaakActionPropertyType.STRING : PazaakActionPropertyType.NUMBER;
      props.push({
        type: type,
        value: properties[i],
      });
