...
export class CursorManager {
  ...
  static setReticle(reticle = 'reticleF') {
    ...
    if (CursorManager.reticle.material != reticleMaterial) CursorManager.reticle.material = reticleMaterial;
  }
  ...
  static setReticle2(reticle = 'reticleF') {
    const reticleMaterial = CursorManager.cursorMaterials.get(reticle);
    if (CursorManager.reticle2.material != reticleMaterial) CursorManager.reticle2.material = reticleMaterial;
  }
  ...
  public static setReticleHoveredObject(object: ModuleObject) {
    ...
    if (BitWise.InstanceOf(object?.objectType, ModuleObjectType.ModuleDoor)) {
      if (canChangeCursor) CursorManager.setCursor('door');
      else CursorManager.setCursor('select');
      ...
      if (canChangeCursor) CursorManager.setCursor('use');
      else CursorManager.setCursor('select');
      ...
      if (canChangeCursor) CursorManager.setCursor('trap');
      else CursorManager.setCursor('select');
      ...
      if (object.isHostile(GameState.getCurrentPlayer())) {
        if (!object.isDead()) {
          if (canChangeCursor) CursorManager.setCursor('attack');
          else CursorManager.setCursor('select');
        ...
        if (canChangeCursor) CursorManager.setCursor('talk');
        else CursorManager.setCursor('select');
  ...
  static updateCursor() {
    ...
    CursorManager.setCursor('default');
    GameState.scene_cursor_holder.position.x =
      Mouse.positionViewport.x - GameState.ResolutionManager.getViewportWidth() / 2 + 32 / 2;
    GameState.scene_cursor_holder.position.y =
      Mouse.positionViewport.y * -1 + GameState.ResolutionManager.getViewportHeight() / 2 - 32 / 2;
    ...
    for (let i = 0; i < controlCount; i++) {
      ...
      if (
        BitWise.InstanceOfObject(control, GUIControlTypeMask.GUIListBox) &&
        CursorManager.MenuManager.hoveredGUIElement == undefined
      ) {
    ...
    if (CursorManager.selectedObject) {
      if (
        CursorManager.selectedObject.position.distanceToSquared(GameState.getCurrentPlayer().position) >
        maxSelectableDistanceSquared
      ) {
        CursorManager.selectedObject = undefined;
    ...
    if (CursorManager.hoveredObject) {
      if (
        CursorManager.hoveredObject.position.distanceToSquared(GameState.getCurrentPlayer().position) >
        maxSelectableDistanceSquared
      ) {
        CursorManager.hoveredObject = undefined;
    ...
    if (
      ...
      CursorManager.selected instanceof OdysseyObject3D &&
      !CursorManager.MenuManager.MenuContainer.bVisible
    ) {
      CursorManager.selected.getWorldPosition(CursorManager.reticle2.position);
