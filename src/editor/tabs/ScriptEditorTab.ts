...
export class ScriptEditorTab extends EditorTab {
  ...
  $centerPane: JQuery<HTMLElement>;
  $bottomPane: JQuery<HTMLElement>;
  $nssContainer: JQuery<HTMLElement>;
  $textEditor: JQuery<HTMLElement>;
  bottomTabManager: EditorTabManager;
  errorLogTab: ScriptErrorLogTab;
  compileLogTab: ScriptCompileLogTab;
  ncsTab: NCSInspectorTab;
  nwScriptParser: any;
  ...
  Show(){
    ...
    this.layout = this.$r_container.layout({
      ...
      onresize_end: (pane: any) => {
        Forge.tabManager.TriggerResize();
      }
    });
  ...
  exportScriptIncludes( text: any ){
    return new Promise( (resolve, reject) => {
      ...
      new AsyncLoop({
        array: includes,
        onLoop: (include: any, asyncLoop: AsyncLoop) => {
          console.log(`include: exporting: ${include}.nss`);
          if(!fs.existsSync(path.join('nwscript', include+'.nss'))){
            ResourceLoader.loadResource(ResourceTypes.nss, include, (d: any) => {
  ...
  static async resolveScriptIncludes( source = '', includes: any[] = [] ){ 
    ...
    for(let i = 0; i < include_count; i++){
      const include_source = await ScriptEditorTab.resolveInclude(include_matches[1].toString());
  ...
  static resolveInclude( name = '' ){
    return new Promise( (resolve, reject) => {
      if(ScriptEditorTab.includeMap.has(name)){
        ...
        ResourceLoader.loadResource(ResourceTypes.nss, name, (d: any) => {
...
export class ScriptErrorLogTab extends EditorTab {
  errors: any[] = [];
  tab: any;
  ...
  update(){
    ...
    for(let i = 0; i < this.errors.length; i++){
      ...
      $error.off('click').on('click', (e: any) => {
        e.preventDefault();
...
export class ScriptCompileLogTab extends EditorTab {
  logs: any[] = [];
  tab : any;
  $logList: JQuery<HTMLElement>;
  constructor( tab: any ){
    super({ closeable: false });
...
export class NCSInspectorTab extends EditorTab {
  logs: any[] = [];
  tab: any = undefined;
  $logList: JQuery<HTMLElement>;
  script: any;
  ...
  update(){
    this.$logList.html('');
    const offset = 13;
    this.script.instructions.forEach( (instruction: any, key: any, map: any) => {
  ...
  decompileInstriction( instruction: any ){
    // switch(instruction.code){
    //   //case 0x
    // }
  }

  setNCSData( buffer: Buffer ){
...
function arg_value_parser( value: any ): any {
