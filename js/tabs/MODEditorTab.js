class MODEditorTab extends EditorTab {
  constructor(file){
    ...
    this.contextMenu.append(new MenuItem({
      ...
      click: (e) => {
        ...
        if(this.$selected_ele){
          ...
          let file_path = dialog.showSaveDialog({
            title: 'Export File',
  ...
  init(){
    ...
    $('.file-browser-item', this.$list).on('click', (e) => {
    ...
    $('.file-browser-item', this.$list).on('contextmenu', (e) => {
      e.preventDefault();
      let $ele = $(e.currentTarget);
      this.$selected_ele = $ele;
