class GUIScrollBar extends GUIControl{
  ...
  mouseInside(){
    ...
    //if(this.inner_box.containsPoint({x: mouseX, y: mouseY})){
      ...
      this.thumb.position.y = -(mouseY + this.thumb.scale.y/2);

      if(this.thumb.position.y < -((scrollBarHeight - this.thumb.scale.y))/2 ){
        this.thumb.position.y = -((scrollBarHeight - this.thumb.scale.y))/2
      }
  ...
  update(){
    ...
    if(this.list){
      ...
      this.list.scroll = Math.floor(this.list.maxScroll * this.scrollPos);
