class GUIListBox extends GUIControl {
  ...
  addItem(node, onClick = null, customBuilder = null){
    ...
    if(typeof customBuilder == 'function'){
      ...
      switch(type){
        case 4:
          control.GetFieldByLabel('TEXT').GetChildStructs()[0].GetFieldByLabel('TEXT').SetValue(this.itemGroup.children.length+1+'. '+node);
  ...
  updateList(){
    ...
    //setTimeout( () => {
      ...
      let nodeOffset = (-this.scroll * this.getNodeHeight()/2);
    ...
    this.maxScroll = this.children.length;
  ...
  cullOffscreen(){
    ...
    let nodePadding = (this.getNodeHeight()/2)+1;
  ...
  getNodeHeight(node = null){
    ...
    if(!node){
      ...
      if(control.textGeometry){
        ...
        let tSize = control.textGeometry.boundingBox.getSize();
        if(tSize.y > cHeight){
          cHeight = tSize.y;
        }
      }
  ...
  scrollUp(){
    /*for(let i = 0; i < this.itemGroup.children.length; i++){
      let node = this.itemGroup.children[i];
      let control = node.control;
      node.position.y -= (control.extent.height + (control.getBorderSize()/2));
    }*/
    this.scroll -= 1;
    if(this.scroll < 0)
      this.scroll = 0;
