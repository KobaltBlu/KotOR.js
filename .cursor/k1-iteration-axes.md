...
**Completeness (retail EXE, ~tens of thousands of binary functions):** **Not** implied by `k1:exhaustive:all`. See [k1-binary-exe-coverage-model.md](k1-binary-exe-coverage-model.md) — private function manifest, **domain** partition, and **N/A** register, orthogonal to the 1490 file checklist.
...
|------|------:|----------------|---------------|
| Per `src/**/*.ts` and `src/**/*.tsx` | **1490** | [k1-iteration-todos-exhaustive.md](k1-iteration-todos-exhaustive.md) | `python .cursor/scripts/verify_k1_iteration_exhaustive.py` (also `diff_exhaustive_src.py`) |
...
| Top-level `src/*` file counts and notes | 29 groups | [k1-src-directory-coverage.md](k1-src-directory-coverage.md) | `python .cursor/scripts/regenerate_k1_src_directory_coverage.py` (must match 1490 total) |
...
**Jest:** `testMatch: **/*.test.ts` in `jest.config.js`. All 62+ `*.test.ts` under `src/` are part of the **1490** `SRC-####` set (no parallel list).

**Game runtime (`src/`):** the **1490** `SRC-####` rows are the authority for `k1_win_gog_swkotor.exe` parity. **Repo satellite** `.ts`/`.tsx` (currently under `extensions/kotor-forge-vscode/`) is tracked as **36** `EXT-####` rows; default **[N/A]** to the retail EXE (editor / LSP). There is no `main/*.ts` at repo root in this tree; bundler config is JS.
...
- **`tsconfig.json`:** `include` is `src/**/*.ts`, `src/**/*.tsx`, and `src/index.d.ts` — the same files enumerated by the **1490** list (and nothing else is typechecked as “app” TS in that config).
