...
**If you’re about to ask again:** this heading already covers the **same** request—**installed software** and **Start Menu + Desktop** shortcuts (not a literal file-by-file crawl of all of `C:\`), **Fedora Kinoite** alternatives with **honest** “almost identical” / **gap** notes, **web** links, and the named apps **ShareX, Steam, Discord, Cloudflare WARP** in **`#### Named apps`**. Refresh **only** after a **big** change to installs: run `winget list` + [`../scripts/list-windows-shortcuts.ps1`](../scripts/list-windows-shortcuts.ps1), then edit **`#### Machine-verified snapshot`**.

> **Nav / scope:** This section is the **only** in-repo, **source-backed** map from **this PC’s** installs (below) to **Fedora Kinoite**-style tooling. **Names you keep asking for:** **ShareX, Steam, Discord, Cloudflare WARP** → see **`#### Named apps (quick ref …)`** and **`#### External references (web)`**, then the **full table** (VPNs, DCC, dev, games, *arr*, etc.). **Re-survey the machine** with `winget list` / `winget export` and [`scripts/list-windows-shortcuts.ps1`](../scripts/list-windows-shortcuts.ps1) after **bulk** install/uninstall.

#### Machine-verified snapshot (this session, 2026-04-25)

| Source | What was run | Result (this host) | Where the raw output lives |
|--------|---------------|---------------------|----------------------------|
| **Winget** | `winget list --accept-source-agreements` | **310** lines (header + table rows) | `%TEMP%\winget-list-2026-04-25.txt` (local only; not committed) |
| **Shortcuts** | [`../scripts/list-windows-shortcuts.ps1`](../scripts/list-windows-shortcuts.ps1) | **17,888** lines written (section headers + one path per line) | `%TEMP%\start-menu-shortcuts-20260425.txt` (local only) |
...
**Collection method (unchanged):** `winget list` (all sources), `Get-ItemProperty` on `HKLM\…\Uninstall` (x64+WOW64), and top-level **`C:\Program Files`**. **Shortcuts:** [`scripts/list-windows-shortcuts.ps1`](../scripts/list-windows-shortcuts.ps1) (writes **`%TEMP%\…`**, not the git tree). Prior full pass: **~17,888** `.lnk`/`.url` under All Users + per-user **Start Menu** and **Desktops**—many rows are **Windows/Store** noise; the **value** is surfacing **third-party** and **game launcher** entries.
...
*Version: 2026-04-25 (revised same day: top-of-§ **“if you’re about to ask again”** pointer + **machine re-check** — **winget 310** lines, **17,888** shortcut-list lines, **web-backed** WARP/Steam/Discord/ShareX **links**). **Phase A = Kinoite in WSL2 (OSTree + rpm-ostree target),** not classic Fedora. **Provisional workspace materialized** under `G:\workspaces\Kinoite` (see that repo’s `WORKSPACE_STATUS.md` and latest `imports/winget-export-*.json` / `windows-inventory-*.txt` for this host).*
